...
def select_residual_threshold(
    ...
    *,
    ...
    """On D_V, maximize net RMR under every preregistered constraint.

    Ties are resolved by total Pd, gross RMR, then the more conservative
    threshold.  The explicit null candidate is residual-off and therefore
    sorts above every numeric threshold when all metric terms tie; this avoids
    silently treating numeric ``1.0`` as an off switch.
    """
