...
class StateCacheRecord:
    ...
    ``reachable_miss_ids`` is the complete sorted catalog of individually
    full-GT-recoverable factual misses, never the singleton selected for one
    training state. Legal and factual candidates are cached, while both
    deterministic uniform choices remain online functions of ``sample_id``,
    epoch, and global seed.
    """
...
def _content_fingerprint(state: StateCacheRecord) -> str:
    ...
    for name in sorted(tensors):
        ...
        # NumPy may expose an empty array to PyTorch with a zero stride.  Such
        # tensors are semantically contiguous but cannot be reinterpreted via
        # ``view(uint8)``.  Their byte payload is empty and shape/dtype are
        # already hashed above, so skip only that impossible reinterpretation.
        if tensor.numel():
