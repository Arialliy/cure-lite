...
class PreprocessConfig:
    """Deterministic evaluation-grid preprocessing bound to a base adapter."""
    ...
    def __post_init__(self) -> None:
        ...
        if any(
            ...
            raise ValueError("normalization statistics must be finite numbers")
        if any(value <= 0 for value in self.std):
            raise ValueError("normalization standard deviations must be positive")
        if self.image_interpolation != "bilinear" or self.mask_interpolation != "nearest":
            raise ValueError("dense evaluation resize requires bilinear/nearest")
    ...
    @classmethod
    def from_fingerprint_payload(
        cls,
        value: object,
    ) -> "PreprocessConfig":
        """Reconstruct the exact generic preprocessing contract from a cache."""
        ...
        if not isinstance(value, dict) or set(value) != expected:
            raise ValueError("preprocessing payload fields are not canonical")
        if value["range"] != "float32-[0,1]-then-normalize":
            raise ValueError("preprocessing tensor range is not supported")
        mean = value["mean"]
        std = value["std"]
        if not isinstance(mean, list) or not isinstance(std, list):
            raise TypeError("preprocessing mean/std must be JSON lists")
        result = cls(
            height=value["height"],  # type: ignore[arg-type]
            width=value["width"],  # type: ignore[arg-type]
            color_mode=value["color_mode"],  # type: ignore[arg-type]
            mean=tuple(mean),
            std=tuple(std),
            image_interpolation=value["image_interpolation"],  # type: ignore[arg-type]
            mask_interpolation=value["mask_interpolation"],  # type: ignore[arg-type]
        )
        if result.fingerprint_payload() != value:
            raise ValueError("preprocessing payload is not canonical")
        return result
