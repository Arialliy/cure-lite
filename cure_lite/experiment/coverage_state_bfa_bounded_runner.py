"""Single-candidate bounded-400 protocol for v20 BFA-CMIF + PMOPE.

The runner preserves the v18 seed, cache, schedule, optimizer, PMOPE
objective, and zero-threshold evaluation.  The only permitted model-side
difference is the predeclared BFA-CMIF field equation.  One invocation runs
training, the mandatory read-only 32-pair diagnostic, and the shared
zero-level evaluation exactly once.
"""
...
from .coverage_state_bfa_certificate import (
    COVERAGE_STATE_BFA_CERTIFICATE_PAIR_COUNT,
    CoverageStateBFACertificateReceipt,
...
from .coverage_state_bfa_decision import (
...
COVERAGE_STATE_BFA_BOUNDED_AUTHORIZATION_SCHEMA = (
    "cure-lite-bfa-cmif-v20-pmope-bounded-run-authorization-v1"
)
COVERAGE_STATE_BFA_BOUNDED_RESULT_SCHEMA = (
    "cure-lite-bfa-cmif-v20-pmope-bounded-run-result-v1"
)
...
COVERAGE_STATE_BFA_BOUNDED_IMPLEMENTATION_PATHS = tuple(
    dict.fromkeys(
        (
            *COVERAGE_STATE_BOUNDED_IMPLEMENTATION_PATHS,
...
def _current_implementation_binding() -> tuple[tuple[str, str], ...]:
    ...
    for relative in COVERAGE_STATE_BFA_BOUNDED_IMPLEMENTATION_PATHS:
        path = root / relative
        if not path.is_file() or path.is_symlink():
            raise RuntimeError(
                f"BFA bounded implementation path is invalid: {relative}"
            )
...
def _model_config_payload(
    ...
    if type(config) is not CoverageStateBinaryFlipAntisymmetricConfig:
        raise TypeError("v20 requires the exact BFA-CMIF config")
    return {
        "config": asdict(config),
        "model_class": "CURELiteBinaryFlipAntisymmetricLevelSet",
...
def _dr_model_config_payload(
    ...
    """Project the model config onto the real-D_R receipt contract."""
...
def _common_initial_model_fingerprints(
    ...
    """Return training-state and raw-state hashes of seed-42 BFA-CMIF."""
...
def expected_coverage_state_bfa_config(
    preflight: CoverageStateBoundedPreflight,
) -> CoverageStateBinaryFlipAntisymmetricConfig:
    """Return the only v20 bounded model configuration."""
...
def verify_current_sealed_v18_negative(
    ...
    """Verify the frozen v18 scientific reference without executing it."""
...
def _dr_gate_binding_payload(
    receipt: CoverageStateBFADRGateReceipt,
    *,
    dataset_free_receipt_fingerprint: str,
    canonical_payload: Mapping[str, object] | None = None,
) -> dict[str, object]:
    if not isinstance(receipt, CoverageStateBFADRGateReceipt):
        raise TypeError("dr_gate_receipt must be a BFA D_R receipt")
    ...
    if (
        values["schema_version"] != COVERAGE_STATE_BFA_DR_GATE_SCHEMA
        ...
        raise PermissionError("BFA D_R gate is not a bound pass")
    return values


@dataclass(frozen=True, eq=False)
class CoverageStateBFABoundedRunAuthorization(
    CoverageStateRunAuthorization,
):
    """All immutable prerequisites for the unique v20 bounded run."""

    preflight: CoverageStateBoundedPreflight
    dataset_free_receipt: CoverageStateBFADatasetFreeReceipt
    dataset_free_receipt_fingerprint: str
    dr_gate_receipt: CoverageStateBFADRGateReceipt
    ...
    def _validate_lightweight_bindings(self) -> None:
        if (
            ...
            raise TypeError("BFA authorization prerequisite type changed")
        expected_config = expected_coverage_state_bfa_config(
            self.preflight
        )
        ...
        if (
            ...
            or self.sealed_v18_receipt_fingerprint
            != COVERAGE_STATE_USCOPE_V18_SEALED_RECEIPT_FINGERPRINT
            ...
            or self.fixed_margin != COVERAGE_STATE_BFA_MARGIN
            or self.coverage_policy
            != CSLF_PHASE_PRESERVING_COVERAGE_POLICY
            or self.interaction_policy != BFA_INTERACTION_POLICY
            or self.energy_policy != BFA_ENERGY_POLICY
            or self.flip_policy != BFA_FLIP_POLICY
            or training_initial
            != COVERAGE_STATE_BFA_HISTORICAL_INITIAL_MODEL_FINGERPRINT
            ...
            raise ValueError("BFA bounded authorization binding changed")
    ...
    def canonical_payload(self) -> dict[str, object]:
        return {
            ...
            "historical_comparison_coordinates": {
                "reference": "sealed_v18_pmope_bounded_400",
    ...
    def verify_unchanged(self) -> None:
        ...
        if stable_fingerprint(
            ...
            raise RuntimeError("BFA bounded authorization changed")
    ...
    def verify_model_config(
        ...
        if (
            type(model_config)
            is not CoverageStateBinaryFlipAntisymmetricConfig
            ...
            raise PermissionError(
                "BFA authorization rejects this model config"
            )
    ...
    def verify_for_run(
        ...
        *,
        ...
        if (
            ...
            raise PermissionError(
                "BFA authorization rejects this bounded run"
            )


def prepare_coverage_state_bfa_bounded_run_authorization(
    preflight: CoverageStateBoundedPreflight,
    dataset_free_receipt: CoverageStateBFADatasetFreeReceipt,
    dr_gate_receipt: CoverageStateBFADRGateReceipt,
    *,
...
) -> CoverageStateBFABoundedRunAuthorization:
    """Bind the generated, real-D_R, and sealed-v18 prerequisites."""
    ...
    if not isinstance(
        ...
        raise TypeError("dataset_free_receipt must be BFA dataset-free")
    if not isinstance(
        dr_gate_receipt,
        CoverageStateBFADRGateReceipt,
    ):
        raise TypeError("dr_gate_receipt must be BFA D_R")
    ...
    model_config = expected_coverage_state_bfa_config(preflight)
    result = CoverageStateBFABoundedRunAuthorization(
        preflight=preflight,
        ...
        fixed_margin=COVERAGE_STATE_BFA_MARGIN,
...
def _bfa_bounded_result_checks(
    authorization: CoverageStateBFABoundedRunAuthorization,
    training: CoverageStateMatchedTrainingResult,
    certificate: CoverageStateBFACertificateReceipt,
    diagnostic: CoverageStateZeroLevelEvaluationResult,
    decision: CoverageStateBFABoundedDecision,
    *,
    ...
    checks = {
        "authorization_and_sealed_v18_reference": (
        ...
        "exact_bfa_model": (
            model is not None
            and type(model)
            is CURELiteBinaryFlipAntisymmetricLevelSet
...
@dataclass(frozen=True, eq=False)
class CoverageStateBFABoundedRunResult:
    """The unique v20 training result plus mandatory read-only reports."""
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ValueError("BFA bounded result is incomplete")
    ...
    def verify_unchanged(self) -> None:
        ...
        if self.decision.decision_fingerprint != stable_fingerprint(
            ...
            raise RuntimeError("BFA bounded decision changed")
        expected = _bfa_bounded_result_checks(
            self.authorization,
            self.training,
            self.certificate,
            self.diagnostic,
        ...
        if self.checks != expected:
            raise RuntimeError("BFA bounded result checks changed")

    def canonical_payload(self) -> dict[str, object]:
        self.verify_unchanged()
        return {
            "schema_version": COVERAGE_STATE_BFA_BOUNDED_RESULT_SCHEMA,
            "run_id": "cure_lite_bfa_cmif_v20_pmope_bounded_400_r1",
            "runtime_splits": ["D_R"],
            "authorization_fingerprint": (
                self.authorization.authorization_fingerprint
            ),
            "candidate_model": "BFA-CMIF",
...
def run_coverage_state_bfa_cmif_pmope_bounded_400(
    authorization: CoverageStateBFABoundedRunAuthorization,
    model_config: CoverageStateBinaryFlipAntisymmetricConfig,
    *,
    device: torch.device | str,
) -> CoverageStateBFABoundedRunResult:
    """Run BFA training -> certificate -> zero-level decision once."""

    if not isinstance(
        authorization,
        CoverageStateBFABoundedRunAuthorization,
    ):
        raise TypeError("authorization must be a BFA authorization")
    if torch.device(device) != torch.device("cuda:0"):
        raise PermissionError(
            "BFA bounded-400 is frozen to visible cuda:0"
        )
    ...
    with _deterministic_execution(device):
        ...
        training = train_matched_coverage_state_bfa_pmope_objectives(
            model_config,
        ...
        if (
            ...
            is not CURELiteBinaryFlipAntisymmetricLevelSet
        ):
            raise RuntimeError(
                "BFA training returned a non-singleton model"
            )
        model = training.models[0][1].eval()
        certificate_invocations += 1
        certificate = audit_coverage_state_bfa_pair_certificate(
            model,
            preflight.population.cache,
            device=device,
            pair_batch_size=COVERAGE_STATE_BFA_CERTIFICATE_PAIR_COUNT,
        )
        ...
        decision = decide_coverage_state_bfa_bounded(diagnostic)
    result = CoverageStateBFABoundedRunResult(
        authorization=authorization,
        training=training,
        ...
        checks=_bfa_bounded_result_checks(
            authorization,
            training,
            certificate,
            diagnostic,
...
__all__ = [
    "COVERAGE_STATE_BFA_BOUNDED_AUTHORIZATION_SCHEMA",
    "COVERAGE_STATE_BFA_BOUNDED_IMPLEMENTATION_PATHS",
    "COVERAGE_STATE_BFA_BOUNDED_RESULT_SCHEMA",
    "COVERAGE_STATE_BFA_HISTORICAL_CACHE_FINGERPRINT",
    "COVERAGE_STATE_BFA_HISTORICAL_DEVICE_CACHE_FINGERPRINT",
    "COVERAGE_STATE_BFA_HISTORICAL_INITIAL_MODEL_FINGERPRINT",
    ...
    "CoverageStateBFABoundedRunAuthorization",
    "CoverageStateBFABoundedRunResult",
    "expected_coverage_state_bfa_config",
    "prepare_coverage_state_bfa_bounded_run_authorization",
    "run_coverage_state_bfa_cmif_pmope_bounded_400",
