"""Read-only post-training pair diagnostics for BFA-CMIF.

The certificate reconstructs the fixed-margin two-endpoint violation and raw
zero-level sign errors for the 16 clean-positive and 16 component-null
optimizer pairs.  These values are mandatory diagnostics for v20, but they
are deliberately not the structural-advancement decision: the latter is
defined in :mod:`coverage_state_bfa_decision`.
"""
...
COVERAGE_STATE_BFA_CERTIFICATE_SCHEMA = (
    "cure-lite-bfa-cmif-v20-post-training-pair-certificate-v1"
)
COVERAGE_STATE_BFA_CERTIFICATE_ROLE_COUNT = 16
COVERAGE_STATE_BFA_CERTIFICATE_PAIR_COUNT = 32
COVERAGE_STATE_BFA_CERTIFICATE_DEFAULT_PAIR_BATCH_SIZE = 4
COVERAGE_STATE_BFA_CERTIFICATE_POLICY = (
    "fixed-32-pair-full-valid-margin-and-raw-zero-level-diagnostic-v1"
)
COVERAGE_STATE_BFA_CERTIFICATE_ROLES = (
    "clean_positive",
    "component_null",
)
...
def _resolve_device(device: torch.device | str) -> torch.device:
    ...
    if result.type not in {"cpu", "cuda"}:
        raise ValueError("BFA certificate device must be CPU or CUDA")
    if result.type == "cuda":
        if result.index is None:
            raise ValueError(
                "BFA certificate CUDA device needs an explicit index"
            )
        if not torch.cuda.is_available():
            raise RuntimeError("CUDA is unavailable")
        if result.index >= torch.cuda.device_count():
            raise ValueError("BFA certificate CUDA device is unavailable")
    return result
...
def _validate_model_device(
    ...
    if not tensors:
        raise ValueError("BFA-CMIF model has no state tensors")
    if any(value.device != device for value in tensors):
        raise ValueError(
            "BFA-CMIF model must already be on the requested device"
        )
    if any(
        value.is_floating_point() and value.dtype != torch.float32
        for value in tensors
    ):
        raise ValueError("BFA-CMIF certificate fixes FP32")
...
def _fixed_optimizer_pairs(
    ...
    if (
        len(clean) != COVERAGE_STATE_BFA_CERTIFICATE_ROLE_COUNT
        or len(component) != COVERAGE_STATE_BFA_CERTIFICATE_ROLE_COUNT
    ):
        raise ValueError(
            "BFA certificate requires exactly 16 clean-positive and "
            "16 component-null optimizer pairs"
        )
    values = tuple(
        sorted(
            (*clean, *component),
    ...
    if (
        len(values) != COVERAGE_STATE_BFA_CERTIFICATE_PAIR_COUNT
        or len({value.record.pair_id for value in values}) != len(values)
        or any(
            value.optimizer_role
            not in COVERAGE_STATE_BFA_CERTIFICATE_ROLES
            for value in values
        )
    ):
        raise ValueError("BFA certificate pair universe changed")
    for value in values:
        ...
        if (
            ...
            raise ValueError("BFA certificate requires singleton records")
    return values
...
def _stack_target_geometry(
    ...
    *,
    ...
    if endpoint not in {"plus", "minus"}:
        raise ValueError("unknown BFA certificate endpoint")
    target = torch.cat(
        tuple(
    ...
    if (
        ...
        raise ValueError(
            "BFA certificate target geometry must be strictly signed"
        )
    return target, valid


@dataclass(frozen=True)
class CoverageStateBFAPairCertificate:
    """One fixed two-endpoint diagnostic."""
...
@dataclass(frozen=True)
class CoverageStateBFACertificateReceipt:
    """Immutable read-only certificate and diagnostic summary."""
    ...
    pair_certificates: tuple[CoverageStateBFAPairCertificate, ...]
    ...
    def verify(self) -> None:
        if (
            self.schema_version != COVERAGE_STATE_BFA_CERTIFICATE_SCHEMA
            or self.certificate_policy
            != COVERAGE_STATE_BFA_CERTIFICATE_POLICY
            or self.clean_positive_count
            != COVERAGE_STATE_BFA_CERTIFICATE_ROLE_COUNT
            ...
            raise ValueError("BFA certificate receipt contract changed")
        ...
        if (
            ...
            raise ValueError("BFA pair certificates are not canonical")
        margin = float.fromhex(self.margin_hex)
        if not isfinite(margin) or margin <= 0.0:
            raise ValueError("BFA certificate margin is invalid")
        for value in self.pair_certificates:
            ...
            if (
                ...
                raise ValueError("BFA pair certificate is inconsistent")
        ...
        if self.integrity_checks != expected_checks:
            raise ValueError("BFA certificate integrity checks changed")
...
def _pair_certificate(
    *,
    ...
    valid_mask: Tensor,
    margin: Tensor,
) -> CoverageStateBFAPairCertificate:
    ...
    if target_sign not in {-1, 1}:
        raise ValueError("BFA worst coordinate has no target sign")
    ...
    return CoverageStateBFAPairCertificate(
        pair_id=cached.record.pair_id,
        sample_id=cached.record.sample_id,
        optimizer_role=cached.optimizer_role,
        gamma_hex=_finite_hex(gamma, name="BFA gamma"),
        gamma_plus_hex=_finite_hex(plus, name="BFA plus gamma"),
        gamma_minus_hex=_finite_hex(minus, name="BFA minus gamma"),
...
def audit_coverage_state_bfa_pair_certificate(
    model: CURELiteBinaryFlipAntisymmetricLevelSet,
    cache: CoverageStateScalarCache,
    *,
    device: torch.device | str,
    pair_batch_size: int = (
        COVERAGE_STATE_BFA_CERTIFICATE_DEFAULT_PAIR_BATCH_SIZE
    ),
) -> CoverageStateBFACertificateReceipt:
    """Report the fixed 32-pair values without modifying model or cache."""
    ...
    if (
        ...
        raise ValueError("BFA model and scalar-cache geometry differ")
    ...
    certificates: list[CoverageStateBFAPairCertificate] = []
        ...
        for start in range(0, len(pairs), pair_batch_size):
            ...
            if (
                ...
                raise FloatingPointError(
                    "BFA certificate fields must be finite FP32"
                )
            ...
            if not torch.equal(valid_plus, valid_minus):
                raise ValueError(
                    "BFA pair endpoints have different valid domains"
                )
            margin = torch.full(
                (),
                margin_value,
    ...
    result = CoverageStateBFACertificateReceipt(
        schema_version=COVERAGE_STATE_BFA_CERTIFICATE_SCHEMA,
        certificate_policy=COVERAGE_STATE_BFA_CERTIFICATE_POLICY,
    ...
    if not result.integrity_passed:
        raise RuntimeError("BFA certificate integrity did not pass")
    return result
