"""D_R-only bounded model-code gate for CURE-Lite D-SVEF v5.
...
runner instantiates a fresh directed-evidence decoder and changes no other
...
from ..directed_factorized_decoder import (
...
DIRECTED_FACTORIZED_OUTCOME_BOUNDED_SCHEMA = (
    "cure-lite-dsvef-v5-bounded-execution-v1"
)
...
def directed_factorized_decoder_state_fingerprint(
    decoder: CURELiteDirectedFactorizedDecoder,
) -> str:
    """Hash v5 class identity, topology, and exact tensor state."""

    if not isinstance(decoder, CURELiteDirectedFactorizedDecoder):
        raise TypeError(
            "decoder must be CURELiteDirectedFactorizedDecoder"
        )
    digest = hashlib.sha256()
    digest.update(b"cure-lite-dsvef-v5-decoder-state-v1")
...
def _validate_directed_execution_inputs(
    ...
    decoder_config: DirectedFactorizedDecoderConfig,
    loss_config: LossConfig,
    optimization_budget: Mapping[str, object],
    device: torch.device | str,
    evaluation_chunk_size: int,
) -> tuple[torch.device, int, float, float]:
    if not isinstance(decoder_config, DirectedFactorizedDecoderConfig):
        raise TypeError(
            "decoder_config must be DirectedFactorizedDecoderConfig"
        )
    ...
    if decoder_config.feature_channels != materializer.feature_shape[1]:
        raise ValueError(
            "directed decoder channels differ from outcome inputs"
        )
    ...
    if expected != (evaluation_height, evaluation_width):
        raise ValueError(
            "bounded D-SVEF requires a native subpixel path without field "
            f"resize ({expected} != "
...
def _structural_failure_result(
    ...
    *,
    ...
    if (
        ...
        raise RuntimeError("malformed failed D-SVEF structural audit")
    result: dict[str, object] = {
        "schema_version": DIRECTED_FACTORIZED_OUTCOME_BOUNDED_SCHEMA,
        "execution_status": "completed",
        "decision": "D_SVEF_STRUCTURAL_EXECUTION_FAIL",
        ...
        "parameters": {
            ...
            "initial_decoder_fingerprint": (
                directed_factorized_decoder_state_fingerprint(decoder)
            ),
        },
        "forward_budget": {
        ...
        "interpretation": {
            "evidence_scope": (
                "fresh_D_SVEF_decoder_pretraining_D_R_structural_audit"
            ),
...
def execute_directed_factorized_outcome_bounded(
    population: OutcomeBoundedAnchorPopulation,
    factual_schedule: OutcomeFactualAnchorSchedule,
    schedule: OutcomePairSchedule,
    materializer: PairedOutcomeInputMaterializer,
    ...
    *,
    ...
    """Train and audit one fresh D-SVEF decoder under 400 frozen updates."""
    ...
    (
        ...
        learning_rate,
        weight_decay,
    ) = _validate_directed_execution_inputs(
        population,
        factual_schedule,
        schedule,
        materializer,
    ...
    decision = (
        "D_SVEF_BOUNDED_MODEL_CODE_GATE_PASS"
        if computational_pass
        else (
            "D_SVEF_STRUCTURAL_EXECUTION_FAIL"
            if not structural_execution_pass
            else "D_SVEF_BOUNDED_MODEL_CODE_GATE_FAIL"
        )
    )
    result: dict[str, object] = {
        "schema_version": DIRECTED_FACTORIZED_OUTCOME_BOUNDED_SCHEMA,
        ...
        "interpretation": {
            "evidence_scope": (
                "fresh_D_SVEF_decoder_bounded_D_R_full_outcome_population"
            ),
            "not_detection_performance_evidence": True,
            "does_not_establish_Pd_or_FA": True,
            "does_not_authorize_formal_training": True,
...
__all__ = [
    "DIRECTED_FACTORIZED_OUTCOME_BOUNDED_SCHEMA",
    "directed_factorized_decoder_state_fingerprint",
