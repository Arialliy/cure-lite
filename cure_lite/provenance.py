...
EXTERNAL_NATIVE_TRAINER_CONTRACT = "external-wrapper-native-train-cli-v1"
OWNED_NATIVE_TRAINER_CONTRACT = "cure-lite-pinned-mshnet-runner-v1"
OWNED_NATIVE_TRAINER_ENTRYPOINT = "run_pinned_mshnet_train.py"
...
def deterministic_base_checkpoint_selection(
    manifest: SplitManifest,
    *,
    select_fraction: float = 0.2,
    seed: int = 0,
    min_fit_samples: int = 1,
    min_select_samples: int = 1,
) -> BaseCheckpointSelection:
    """Partition ``D_B`` deterministically without splitting related samples.

    Every ``SplitRecord.grouping_keys()`` relation is treated as an edge.  The
    resulting transitive connected components are indivisible, so a chain such
    as ``same group_id`` followed by ``same scene_id`` cannot leak across
    ``D_B-fit`` and ``D_B-select``.  Among the component-level subset sizes that
    satisfy the requested minima, the selected subset is the one closest to
    ``select_fraction``.  A content hash seeded by ``seed`` provides a stable
    tie order independent of manifest row order and Python hash randomization.

    The returned :class:`BaseCheckpointSelection` remains the sole persisted
    contract.  Callers that run several training seeds should keep this
    partition ``seed`` fixed so checkpoint comparisons use identical samples.
    """

    if not isinstance(manifest, SplitManifest):
        raise TypeError("manifest must be a SplitManifest")
    if isinstance(select_fraction, bool):
        raise BaseTrainingProvenanceError(
            "select_fraction must be finite and strictly between zero and one"
        )
    try:
    ...
    if not isinstance(seed, int) or isinstance(seed, bool):
        raise BaseTrainingProvenanceError("seed must be an integer")
    for name, value in (
        ("min_fit_samples", min_fit_samples),
        ("min_select_samples", min_select_samples),
    ):
        if not isinstance(value, int) or isinstance(value, bool) or value < 1:
            raise BaseTrainingProvenanceError(f"{name} must be a positive integer")

    records = tuple(sorted(manifest.records_for("D_B"), key=lambda row: row.sample_id))
    manifest.assert_purpose("base_train", records)
    sample_count = len(records)
    if sample_count < min_fit_samples + min_select_samples:
        raise BaseTrainingProvenanceError(
            "D_B is too small for the requested fit/select sample minima"
        )

    # Union every direct grouping-key match.  Components are recovered only
    # after all unions, which also protects transitive relations.
    ...
    def union(left: int, right: int) -> None:
        ...
        if left_root == right_root:
            return
        # Root choice is deterministic even if record traversal changes later.
        if left_root < right_root:
            parents[right_root] = left_root
        else:
            parents[left_root] = right_root

    grouping_owner: dict[tuple[str, str], int] = {}
    for index, record in enumerate(records):
        for grouping_key in record.grouping_keys():
            prior = grouping_owner.setdefault(grouping_key, index)
            union(index, prior)

    component_members: dict[int, list[str]] = {}
    for index, record in enumerate(records):
        component_members.setdefault(find(index), []).append(record.sample_id)
    components = tuple(
        sorted(tuple(sorted(sample_ids)) for sample_ids in component_members.values())
    )
    if len(components) < 2:
        raise BaseTrainingProvenanceError(
            "D_B has fewer than two grouping-key connected components; "
            "a group-disjoint fit/select partition is impossible"
        )

    def component_order_key(sample_ids: tuple[str, ...]) -> tuple[str, tuple[str, ...]]:
        encoded = json.dumps(
            {"seed": seed, "sample_ids": list(sample_ids)},
            sort_keys=True,
            separators=(",", ":"),
        ).encode("utf-8")
        return hashlib.sha256(encoded).hexdigest(), sample_ids

    ordered_components = tuple(sorted(components, key=component_order_key))
    minimum_select = min_select_samples
    maximum_select = sample_count - min_fit_samples
    desired_select = math.floor(sample_count * fraction + 0.5)
    desired_select = min(max(desired_select, minimum_select), maximum_select)

    # Bounded subset-sum over component sizes.  One immutable backpointer per
    # attainable count is sufficient: future reachability depends on the count,
    # not on which already-processed components produced it.
    backpointers: dict[int, tuple[int, int] | None] = {0: None}
    for component_index, component in enumerate(ordered_components):
        component_size = len(component)
        attainable_before = tuple(sorted(backpointers, reverse=True))
        additions: dict[int, tuple[int, int]] = {}
        for prior_count in attainable_before:
            next_count = prior_count + component_size
            if next_count > maximum_select:
                continue
            if next_count not in backpointers and next_count not in additions:
                additions[next_count] = (prior_count, component_index)
        backpointers.update(additions)

    feasible_counts = tuple(
        count
        for count in backpointers
        if minimum_select <= count <= maximum_select
    )
    if not feasible_counts:
        raise BaseTrainingProvenanceError(
            "grouping-key components cannot satisfy the requested fit/select minima"
        )
    selected_count = min(
        feasible_counts,
        key=lambda count: (abs(count - desired_select), count),
    )

    selected_component_indices: set[int] = set()
    cursor = selected_count
    while cursor:
        pointer = backpointers[cursor]
        if pointer is None:  # pragma: no cover - defensive corruption guard
            raise RuntimeError("invalid component subset backpointer")
        prior_count, component_index = pointer
        selected_component_indices.add(component_index)
        cursor = prior_count
    ...
    select_id_set = set(select_ids)
    fit_ids = tuple(
        record.sample_id
        for record in records
        if record.sample_id not in select_id_set
    )
    return BaseCheckpointSelection.from_manifest(
        manifest,
...
def _single_command_option(command: list[str], option: str) -> str:
    """Return one explicitly supplied command option value."""

    positions = [index for index, value in enumerate(command) if value == option]
    if len(positions) != 1 or positions[0] + 1 >= len(command):
        raise BaseTrainingProvenanceError(
            f"native command must contain exactly one explicit {option}"
        )
    value = command[positions[0] + 1]
    if value.startswith("--"):
        raise BaseTrainingProvenanceError(
            f"native command {option} must have an explicit value"
        )
    return value


def _resolve_regular_file(path: Path, *, name: str) -> Path:
    """Resolve a named source file without accepting a symbolic-link path."""

    if path.is_symlink():
        raise BaseTrainingProvenanceError(f"{name} may not be a symlink")
...
def validate_formal_base_training_run(
    ...
    trainer_contract_name = trainer_contract.get("contract")
    owned_runner_sources: dict[str, str] | None = None
    if trainer_contract_name == EXTERNAL_NATIVE_TRAINER_CONTRACT:
        required_trainer = {
            "contract": EXTERNAL_NATIVE_TRAINER_CONTRACT,
    ...
    elif trainer_contract_name == OWNED_NATIVE_TRAINER_CONTRACT:
        ...
        if set(trainer_contract) != expected_contract_fields:
            missing = sorted(expected_contract_fields - set(trainer_contract))
            unknown = sorted(set(trainer_contract) - expected_contract_fields)
            raise BaseTrainingProvenanceError(
                "invalid CURE-Lite runner contract fields; "
                f"missing={missing}, unknown={unknown}"
            )
        if trainer_contract.get("entrypoint") != OWNED_NATIVE_TRAINER_ENTRYPOINT:
            raise BaseTrainingProvenanceError(
                "formal base CURE-Lite runner entrypoint mismatch"
            )
        _sha256(
            trainer_contract.get("runner_sha256"),
            name="native_trainer_contract.runner_sha256",
        )
        raw_sources = trainer_contract.get("upstream_sources_sha256")
        if not isinstance(raw_sources, Mapping) or set(raw_sources) != set(
            OWNED_MSHNET_SOURCE_PATHS
        ):
            raise BaseTrainingProvenanceError(
                "CURE-Lite runner must bind the MSHNet model, loss, and data sources"
            )
        owned_runner_sources = {
            relative: _sha256(
                raw_sources.get(relative),
                name=f"native_trainer_contract.upstream_sources_sha256[{relative!r}]",
            )
            for relative in OWNED_MSHNET_SOURCE_PATHS
        }
        if final.get("native_trainer_contract") != dict(trainer_contract):
            raise BaseTrainingProvenanceError(
                "preflight/final CURE-Lite runner contracts differ"
            )
    else:
        raise BaseTrainingProvenanceError(
            f"unsupported formal base native-trainer contract {trainer_contract_name!r}"
        )
    ...
    dataset_directory = _single_command_option(command, "--dataset-dir")
    if Path(dataset_directory).resolve() != view_root:
        raise BaseTrainingProvenanceError(
            "native command does not use the isolated dataset view"
        )
    if trainer_contract_name == EXTERNAL_NATIVE_TRAINER_CONTRACT:
        for option, expected in (("--mode", "train"), ("--if-checkpoint", "false")):
            if _single_command_option(command, option) != expected:
                raise BaseTrainingProvenanceError(
        ...
        native_output_raw = _single_command_option(command, "--save-dir")
    else:
        disallowed_owned_options = {
            "--checkpoint",
            "--checkpoint-path",
            "--if-checkpoint",
            "--input-checkpoint",
        ...
        supplied_option_names = {
            item.split("=", 1)[0] for item in command if item.startswith("--")
        }
        present_disallowed = sorted(
            supplied_option_names & disallowed_owned_options
        )
        if present_disallowed:
            raise BaseTrainingProvenanceError(
                "CURE-Lite runner command includes a resume or input-weight option: "
                f"{present_disallowed}"
            )
        runner_path = _resolve_regular_file(
            Path(command[1]), name="CURE-Lite runner"
        )
        if file_sha256(runner_path) != trainer_contract.get("runner_sha256"):
            raise BaseTrainingProvenanceError(
                "CURE-Lite runner SHA256 differs from the preflight record"
            )
        mshnet_repo_raw = _single_command_option(command, "--mshnet-repo")
        mshnet_repo_candidate = Path(mshnet_repo_raw)
        if mshnet_repo_candidate.is_symlink():
            raise BaseTrainingProvenanceError(
                "CURE-Lite runner MSHNet repository may not be a symlink"
            )
        mshnet_repo = mshnet_repo_candidate.resolve(strict=True)
        if not mshnet_repo.is_dir():
            raise BaseTrainingProvenanceError(
                "CURE-Lite runner MSHNet repository must be a directory"
            )
        assert owned_runner_sources is not None
        for relative, declared_sha256 in owned_runner_sources.items():
            source_path = _resolve_regular_file(
                mshnet_repo / relative,
                name=f"MSHNet source {relative}",
            )
            if file_sha256(source_path) != declared_sha256:
                raise BaseTrainingProvenanceError(
                    f"MSHNet source {relative} SHA256 differs from the preflight record"
                )
        native_output_raw = _single_command_option(command, "--output-dir")
    native_output = Path(native_output_raw).resolve(strict=True)
    try:
    ...
    if trainer_contract_name == OWNED_NATIVE_TRAINER_CONTRACT:
        assert owned_runner_sources is not None
        for relative, expected_sha256 in owned_runner_sources.items():
            declared_sha256 = _sha256(
                sources.get(relative),
                name=f"upstream.tracked_python_sources_sha256[{relative!r}]",
            )
            if not hmac.compare_digest(declared_sha256, expected_sha256):
                raise BaseTrainingProvenanceError(
                    f"MSHNet source {relative} differs between upstream and runner records"
                )
...
__all__ = [
    ...
    "EXTERNAL_NATIVE_TRAINER_CONTRACT",
    "BaseCheckpointSelection",
    "BaseTrainingProvenance",
