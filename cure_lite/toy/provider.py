...
class ToyFrozenBaseAdapter(FrozenBaseAdapter):
    ...
    def validate_preprocessing(self, preprocessing: object) -> None:
        if not isinstance(preprocessing, PreprocessConfig):
            raise TypeError("toy preprocessing must be a PreprocessConfig")
        if (
            preprocessing.color_mode != "L"
            or preprocessing.mean != (0.0,)
            or preprocessing.std != (1.0,)
        ):
            raise ValueError(
                "toy base requires a single-channel, unnormalized [0,1] input"
            )
