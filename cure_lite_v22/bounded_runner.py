"""One-shot D_R-only bounded-400 runner for CURE-Lite v22 PACRE.
...
``train_pacre_pmope_candidate``.  The trained checkpoint is then evaluated at
...
from .factory import build_pacre_training_model
from .pacre import (
...
PACRE_BOUNDED_AUTHORIZATION_SCHEMA: Final = (
    "cure-lite-v22-pacre-pmope-bounded-400-authorization-v1"
)
PACRE_BOUNDED_RESULT_SCHEMA: Final = (
    "cure-lite-v22-pacre-pmope-bounded-400-result-v1"
)
...
PACRE_BOUNDED_ATTEMPT_IDENTITY_SCHEMA: Final = (
    "cure-lite-v22-pacre-pmope-bounded-400-attempt-identity-v1"
)
...
def _package_source_inventory() -> tuple[str, ...]:
    ...
    ``cure_lite_v22`` packages.  This is stricter than a hand-maintained import
    list and therefore also covers dynamic imports, package initializers, and
    transitive data/cache/geometry helpers.
    """
...
def _model_binding(
    ...
    with _MODEL_BINDING_LOCK:
        ...
        with torch.random.fork_rng(devices=[]):
            ...
            model = build_pacre_training_model(config)
...
class CoverageStatePACREBoundedRunAuthorization(
    ...
    def verify_unchanged(self) -> None:
        ...
        if (
            ...
            or type(self.model_config) is not CoverageStatePACREConfig
    ...
    def canonical_payload(self) -> dict[str, object]:
        ...
        return {
            ...
            "D_R_gate": self.dr_gate_receipt.canonical_payload(),
    ...
    def _training_binding_fingerprint(
        ...
        *,
        ...
        if (
            type(model_config) is not CoverageStatePACREConfig
        ...
        return stable_fingerprint(
            {
                "schema_version": (
                    "cure-lite-v22-pacre-bounded-training-binding-v1"
                ),
                "run_id": self.run_id,
                "attempt_fingerprint": self.attempt_fingerprint,
...
def prepare_pacre_bounded_run_authorization(
    ...
    *,
    ...
    attempt_binding_fingerprint = stable_fingerprint(
        {
            "schema_version": (
                "cure-lite-v22-pacre-bounded-attempt-binding-v1"
            ),
            "run_id": run_id,
    ...
    *,
    ...
    checks = {
        ...
        "02_exact_pacre_single_candidate": (
            ...
            is
            CURELitePhaseAlignedCenteredResidualCompatibilityEnergyLevelSet
            and type(training.model_config) is CoverageStatePACREConfig
    ...
    *,
    ...
    try:
        with _deterministic_execution(resolved_device):
            ...
            training = train_pacre_pmope_candidate(
                model_config,
