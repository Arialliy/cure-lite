"""Independent single-candidate PMOPE training entry for v23 PACRE-VC.

This module does not use the inheritance-based legacy model registry or any
matched-suite result.  It constructs PACRE-VC only through the strict v23
factory, creates the frozen Adam optimizer, and calls the existing public
single-objective training function with the fixed PMOPE objective.  The
10x40 protected path can be opened only by the exact v23 bounded
authorization; the 800x40 path is deliberately not implemented.
"""
...
PACRE_PMOPE_TRAINING_SCHEMA: Final = (
    "cure-lite-v23-pacre-vc-pmope-training-receipt-v1"
)
...
PACRE_PARAMETER_NAMES: Final = PACRE_VC_PARAMETER_NAMES

# Version-explicit spellings are the public v23 contract.  The shorter names
# remain local compatibility aliases for the generic bounded machinery.
PACRE_VC_PMOPE_TRAINING_SCHEMA: Final = PACRE_PMOPE_TRAINING_SCHEMA
PACRE_VC_PMOPE_TRAINING_SEED: Final = PACRE_PMOPE_TRAINING_SEED
PACRE_VC_PMOPE_OBJECTIVE: Final = PACRE_PMOPE_OBJECTIVE
PACRE_VC_MODEL_FQCN: Final = PACRE_MODEL_FQCN
PACRE_VC_CONFIG_FQCN: Final = PACRE_CONFIG_FQCN
PACRE_VC_OPTIMIZER_FQCN: Final = PACRE_OPTIMIZER_FQCN
PACRE_VC_SOURCE_PATHS: Final = PACRE_SOURCE_PATHS
...
def _source_hashes() -> tuple[tuple[str, str], ...]:
    ...
    paths = {
        "cure_lite_v23/pacre_vc.py": package_root / "pacre_vc.py",
        "cure_lite_v23/factory.py": package_root / "factory.py",
        "cure_lite_v23/training.py": package_root / "training.py",
    }
...
class PACREPMOPETrainingConfig:
    """Frozen single-seed Adam policy for the v23 candidate."""
...
class PACREPMOPETrainingBundle:
    ...
    def bundle_fingerprint(self) -> str:
        ...
        return stable_fingerprint(
            {
                "schema_version": (
                    "cure-lite-v23-pacre-vc-pmope-training-bundle-v1"
                ),
                "receipt_fingerprint": (
...
def _reserve_protected_training_before_allocation(
    ...
    authorization_type = type(authorization)
    if (
        authorization_type.__module__
        != "cure_lite_v23.bounded_runner"
        or authorization_type.__qualname__
...
def train_pacre_vc_pmope_candidate(
    model_config: object,
    cache: CoverageStateScalarCache,
    schedule: CoverageStateTrainingSchedule,
    *,
...
PACREVCPMOPETrainingConfig = PACREPMOPETrainingConfig
PACREVCPMOPETrainingReceipt = PACREPMOPETrainingReceipt
PACREVCPMOPETrainingBundle = PACREPMOPETrainingBundle
PACREVCParameterTopology = PACREParameterTopology
PACRE_VC_PMOPE_TRAINING_CONFIG: Final = PACRE_PMOPE_TRAINING_CONFIG

# Kept as an import-compatible alias for generic callers; it still constructs
# only the exact v23 wrapper through ``build_pacre_vc_training_model``.
train_pacre_pmope_candidate = train_pacre_vc_pmope_candidate
...
__all__ = [
    ...
    "PACRE_SOURCE_PATHS",
    "PACRE_VC_CONFIG_FQCN",
    "PACRE_VC_MODEL_FQCN",
    "PACRE_VC_OPTIMIZER_FQCN",
    "PACRE_VC_PMOPE_OBJECTIVE",
    "PACRE_VC_PMOPE_TRAINING_CONFIG",
    "PACRE_VC_PMOPE_TRAINING_SCHEMA",
    "PACRE_VC_PMOPE_TRAINING_SEED",
    "PACRE_VC_SOURCE_PATHS",
    "PACREPMOPETrainingBundle",
    "PACREPMOPETrainingConfig",
    "PACREPMOPETrainingReceipt",
    "PACREParameterTopology",
    "PACREVCPMOPETrainingBundle",
    "PACREVCPMOPETrainingConfig",
    "PACREVCPMOPETrainingReceipt",
    "PACREVCParameterTopology",
