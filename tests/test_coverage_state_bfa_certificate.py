from __future__ import annotations

import torch

from cure_lite.coverage_state_binary_flip_antisymmetric import (
    CURELiteBinaryFlipAntisymmetricLevelSet,
    CoverageStateBinaryFlipAntisymmetricConfig,
)
from cure_lite.coverage_state_centered_mixed_interaction import (
    CURELiteCenteredMixedInteractionLevelSet,
    CoverageStateCenteredMixedInteractionConfig,
)
from cure_lite.experiment.coverage_state_bfa_certificate import (
    COVERAGE_STATE_BFA_CERTIFICATE_PAIR_COUNT,
    audit_coverage_state_bfa_pair_certificate,
)
from tests_v15.coverage_state_training_test_helpers import (
    make_bounded_training_scalar_cache,
)


def _bfa_model(cache):
    return CURELiteBinaryFlipAntisymmetricLevelSet(
        CoverageStateBinaryFlipAntisymmetricConfig(
            feature_channels=(
                cache.clean_positive_records[0].record.feature.shape[1]
            ),
            feature_stride=cache.raw_catalog.feature_stride,
            width=4,
        )
    )


def test_bfa_certificate_reports_pair_failures_without_failing_integrity(
) -> None:
    cache = make_bounded_training_scalar_cache()
    model = _bfa_model(cache).eval()
    state_before = {
        name: value.detach().clone()
        for name, value in model.state_dict().items()
    }
    cpu_rng_before = torch.random.get_rng_state().clone()

    receipt = audit_coverage_state_bfa_pair_certificate(
        model,
        cache,
        device="cpu",
        pair_batch_size=COVERAGE_STATE_BFA_CERTIFICATE_PAIR_COUNT,
    )

    assert receipt.integrity_passed
    assert len(receipt.pair_certificates) == 32
    assert receipt.model_forward_invocations == 1
    assert receipt.pair_batch_size == 32
    assert not receipt.all_pairs_passed
    assert receipt.total_raw_sign_error_pixels > 0
    assert receipt.canonical_payload()["diagnostic_summary"][
        "pair_result_is_bounded_gate"
    ] is False
    assert torch.equal(cpu_rng_before, torch.random.get_rng_state())
    for name, value in model.state_dict().items():
        assert torch.equal(value, state_before[name])
    receipt.verify()


def test_bfa_certificate_requires_exact_bfa_model() -> None:
    cache = make_bounded_training_scalar_cache()
    model = CURELiteCenteredMixedInteractionLevelSet(
        CoverageStateCenteredMixedInteractionConfig(
            feature_channels=(
                cache.clean_positive_records[0].record.feature.shape[1]
            ),
            feature_stride=cache.raw_catalog.feature_stride,
            width=4,
        )
    )

    try:
        audit_coverage_state_bfa_pair_certificate(
            model,  # type: ignore[arg-type]
            cache,
            device="cpu",
        )
    except TypeError as error:
        assert "CURELiteBinaryFlipAntisymmetricLevelSet" in str(error)
    else:
        raise AssertionError("non-BFA model was accepted")


def test_bfa_certificate_rejects_invalid_batch_size() -> None:
    cache = make_bounded_training_scalar_cache()
    model = _bfa_model(cache)

    try:
        audit_coverage_state_bfa_pair_certificate(
            model,
            cache,
            device="cpu",
            pair_batch_size=0,
        )
    except ValueError as error:
        assert "integer in [1, 32]" in str(error)
    else:
        raise AssertionError("zero pair batch size was accepted")
