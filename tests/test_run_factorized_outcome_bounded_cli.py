...
from tools import run_factorized_outcome_bounded as runner
...
def _fake_structural_audit(
    *,
    ...
    nonfinite = 0
    if not all_pass:
        checks["all_audited_fields_finite"] = False
        nonfinite = 1
    clean_records = [
        {
...
def _fake_core_result() -> dict[str, object]:
    ...
    result: dict[str, object] = {
        ...
        "decision": "SVEF_BOUNDED_MODEL_CODE_GATE_PASS",
...
def _fake_computational_fail_result() -> dict[str, object]:
    ...
    result["decision"] = "SVEF_BOUNDED_MODEL_CODE_GATE_FAIL"
...
def _fake_post_training_structural_fail_result() -> dict[str, object]:
    result = _fake_core_result()
    result["decision"] = "SVEF_STRUCTURAL_EXECUTION_FAIL"
...
def test_frozen_config_and_proposal_bindings_validate() -> None:
    ...
    assert runner.CONFIG_FILE_SHA256 == (
        "0462ecaa3cda30bbb8d8a57d1c4da396"
        "374607cdea6b2aa08cd43cc752ff3cc0"
    )
...
def test_config_and_proposal_semantic_tampering_is_rejected() -> None:
    ...
    changed_proposal["scope"]["D_T_access_allowed"] = True
...
def test_mocked_success_publishes_all_receipts_and_round_trips(
    ...
    published = runner.load_factorized_outcome_bounded_artifact(output)
    ...
    assert complete["artifact_file_count"] == 10
...
def test_failed_final_validation_preserves_incomplete_marker(
    ...
    monkeypatch.setattr(
        runner,
        "load_factorized_outcome_bounded_artifact",
...
def test_loader_rejects_coherently_resigned_decision_tampering(
    ...
    with pytest.raises(RuntimeError, match="result and publication decision"):
        runner.load_factorized_outcome_bounded_artifact(output)
