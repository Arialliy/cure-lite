...
class _RecordingCCFRDecoder(CURELiteCoverageFeatureReleaseDecoder):
...
def test_existing_4_4_2_step_uses_one_2b_pair_forward_and_all_gradients() -> None:
...
def test_identical_model_inputs_cannot_be_dispatched_by_pair_role() -> None:
...
def test_pair_only_gradient_is_the_sum_of_both_endpoint_vjps() -> None:
    with torch.random.fork_rng(devices=[]):
        torch.manual_seed(10101)
        ...
        # Isolate PECO's crossing term itself.  The separate 4/4/2 test
        # above covers the combined factual + pair training objective.
        ...
        parameters = tuple(decoder.parameters())
        clean_plus_output_gradient = torch.zeros_like(endpoint_plus)
        clean_minus_output_gradient = torch.zeros_like(endpoint_minus)
        clean_plus_output_gradient[0].copy_(endpoint_plus[0].detach())
        clean_minus_output_gradient[0].copy_(endpoint_minus[0].detach())
        clean_plus_vjp = torch.autograd.grad(
            plus,
            parameters,
            grad_outputs=clean_plus_output_gradient,
            retain_graph=True,
        )
        clean_minus_vjp = torch.autograd.grad(
            minus,
            parameters,
            grad_outputs=clean_minus_output_gradient,
            retain_graph=True,
        )
    ...
    assert bool(torch.isfinite(endpoint_plus).all())
    assert bool(torch.isfinite(endpoint_minus).all())
    ...
    for gradients in (clean_plus_vjp, clean_minus_vjp):
        assert all(bool(torch.isfinite(value).all()) for value in gradients)
        assert all(
            float(value.detach().double().norm()) > 1.0e-8
