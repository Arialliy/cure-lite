...
from tools import (
    cure_lite_v24_actual_runtime_release_preaccess_compat_c2 as compat,
)


def _valid_r12_release_closure() -> dict[str, object]:
...
def test_c2_identity_is_disjoint_and_retains_scientific_r2(
    ...
    assert "compat_c2" in identity["runtime_spec_path"]
    assert identity["runtime_spec_path"] != str(
        compat.C1_RUNTIME_PATHS[0]
    )
...
@pytest.mark.parametrize("lane", ("direct", "c1"))
...
def test_build_phase_requires_every_c2_future_path_absent(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    future_name: str,
) -> None:
...
def test_authorize_requires_scientific_outputs_pristine(
    ...
    with pytest.raises(PermissionError, match="before c2 launch"):
...
def test_build_and_authorize_phases_use_exact_bridge_modes(
    ...
    monkeypatch.setattr(
        ...
        or {"runtime_compatibility_id": "c2"},
    )
...
def test_environment_cross_binding_calls_authoritative_validator(
    ...
    monkeypatch.setattr(
        compat.compat_environment,
        "validate_c2_realization_archival",
    ...
    monkeypatch.setattr(
        ...
        lambda value: normalized_old if value is old else normalized_c2,
        raising=False,
    )
    receipt = {
        "historical_environment_contract": normalized_old,
        "current_environment_contract": normalized_c2,
    }
    ...
    assert calls[0][-1]["c2_contract"] is c2
...
def test_release_closure_orders_bridge_before_frozen_and_archival(
    ...
    monkeypatch.setattr(
        compat,
        "_require_final_r12_supervisor_binding",
        lambda _result: events.append("r12"),
    )
    monkeypatch.setattr(
        compat.compat_realizer,
        "validate_archival_realization_chain",
...
def test_r12_binding_accepts_current_final_supervisor_generation() -> None:
    compat._require_final_r12_supervisor_binding(
        _valid_r12_release_closure()
    )
...
def test_r12_binding_rejects_stale_supervisor_generation(
    field: str,
) -> None:
    closure = _valid_r12_release_closure()
    binding = closure["integration"]["authorization"][
        "executable_bindings"
    ]["supervisor"]
    if field in {"path", "resolved_path"}:
        binding[field] = "/tmp/stale-c2-supervisor.py"
    elif field == "file_sha256":
        binding[field] = "0" * 64
    else:
        binding[field] += 1
    with pytest.raises(PermissionError, match="r12 integration"):
        compat._require_final_r12_supervisor_binding(closure)


@pytest.mark.parametrize(
    "location",
    ("authorization", "scenario_root"),
)
def test_r12_binding_rejects_wrong_fixed_root(location: str) -> None:
    closure = _valid_r12_release_closure()
    integration = closure["integration"]
    if location == "authorization":
        integration["identities"]["authorization"]["path"] = (
            "/tmp/stale-r12/control/authorization.json"
        )
    else:
...
def test_r12_binding_rejects_non_single_link_current_supervisor(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    closure = _valid_r12_release_closure()
...
def test_r12_and_c2_environment_inputs_are_fixed() -> None:
    assert compat.INTEGRATION_ROOT.name.endswith("compat_c2_r12")
    assert "compat_c2" in compat.POLICY_PATH.name
    assert "compat_c2" in compat.STABILITY_PATH.name
    assert "compat_c2" in compat.POSTCLEANUP_PATH.name
    assert "compat_c2" in compat.REALIZATION_AUTHORIZATION_PATH.name
    assert "compat_c2" in compat.REALIZATION_RECEIPT_PATH.name
...
def test_source_declares_no_retry_resume_or_dv_dt_authority() -> None:
    source = compat.COMPAT_BRIDGE_PATH.parent / (
        "cure_lite_v24_actual_runtime_release_preaccess_compat_c2.py"
    )
