...
from tools import (
    cure_lite_v24_runtime_environment_preaccess_compat_c4 as compat,
)
...
GPU_UUID = "GPU-12cdabd0-7910-8f4a-e4d7-e4c7867d1296"
...
def _archival(
    ...
    *,
    ...
    fragment = {
        "path": str(compat.C4_FRAGMENT_PATH),
        "file_sha256": "f" * 64,
    }
    shadow = {
    ...
    return {
        "authorization": {
            "unit_name": compat.C4_TARGET_UNIT,
...
def _validator(archival):
    def validate(authorization_path: Path, receipt_path: Path):
        assert authorization_path == (
            compat.C4_REALIZATION_AUTHORIZATION_PATH
        )
        assert receipt_path == compat.C4_REALIZATION_RECEIPT_PATH
...
def _inventory(contract) -> dict[str, object]:
    ...
    body: dict[str, object] = {
        ...
        "unit_scope": {
            "target_unit_id": compat.C4_TARGET_UNIT,
            "conflict_unit_ids": list(compat.CONFLICT_UNIT_IDS),
            "dependency_unit_ids": [],
            "require_target_ready": True,
            "shadows": {
                compat.C4_TARGET_UNIT: _shadow(
                    compat.C4_TARGET_UNIT,
                    target=True,
                ),
...
def test_frozen_source_is_hash_before_exec_loaded() -> None:
    assert (
        ...
        == "a40465786ce4537346372df5991bb6788d44feddfd497ec83a1dc402fb8b2fea"
    )
...
def test_old_scope_replay_then_only_target_and_readiness_handoff() -> None:
    old = _contract()
    old_returned, c4, roots = compat.replay_old_scope_and_handoff(
    ...
    assert c4.target_unit_id == compat.C4_TARGET_UNIT
    assert c4.require_target_ready is True
    for field, old_value in asdict(old_returned).items():
        if field not in {"target_unit_id", "require_target_ready"}:
            assert asdict(c4)[field] == old_value
    assert roots == _roots()
...
def test_old_scope_replay_rejects_scope_drift(drift: str) -> None:
    old = _contract()
    if drift == "target":
        old = replace(old, target_unit_id=compat.C4_TARGET_UNIT)
    elif drift == "require":
...
def test_policy_is_fresh_c4_scope(
    ...
    compat.validate_c4_realization_archival(archival, contract=c4)


def test_realization_bridge_source_is_production_frozen() -> None:
    digest = hashlib.sha256(compat.C4_BRIDGE_PATH.read_bytes()).hexdigest()
    assert compat.C4_BRIDGE_SHA256 == digest
    assert compat.C4_BRIDGE_SHA256 != "__TO_BE_FROZEN__"
    module = compat._load_verified_c4_bridge()
...
def test_stability_uses_frozen_gate_directly_and_exact_2x30(
    ...
    def gate(*args, **kwargs):
        ...
        assert kwargs["target_unit_id"] == compat.C4_TARGET_UNIT
    ...
    result = compat.run_c4_stability_in_memory(
        policy,
        realization_validator=_validator(archival),
...
def test_postcleanup_reuses_exact_final_inventory(
    ...
    post = compat.build_c4_postcleanup_in_memory(
        policy,
        stability,
        realization_validator=_validator(archival),
        prepare=_prepare(old, roots),
    ...
    closed = compat.validate_c4_environment_closure(
        policy,
        stability,
        post,
        archival=archival,
...
def test_cross_binding_rejects_resealed_or_archival_drift(
    ...
    if drift == "target":
        ...
        supplied_contract = replace(c4, require_target_ready=False)
    ...
    with pytest.raises((PermissionError, ValueError)):
        compat.validate_c4_environment_closure(
            policy,
            stability,
            None,
            archival=archival,
            c4_contract=supplied_contract,
        )
...
def test_frozen_source_and_r4_pin_reject_drift(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    with monkeypatch.context() as scoped:
        ...
        with pytest.raises(PermissionError):
            compat._require_frozen_environment_source()
    with monkeypatch.context() as scoped:
        scoped.setattr(compat, "C4_BRIDGE_SHA256", "__TO_BE_FROZEN__")
        with pytest.raises(PermissionError):
            compat._load_verified_c4_bridge()
...
def test_main_rejects_fixed_path_identity_drift_before_build(
    ...
    monkeypatch.setattr(compat, "build_c4_policy_in_memory", build)
    monkeypatch.setattr(
        compat,
        "C4_POLICY_PATH",
        tmp_path / "runtime_environment_policy_preaccess_compat_c4.json",
    )
...
def test_main_enforces_policy_stability_postcleanup_order_before_payload(
    ...
    paths = (
        compat.C4_POLICY_PATH,
        compat.C4_STABILITY_PATH,
        compat.C4_POSTCLEANUP_PATH,
    )
...
def _isolated_compatibility_module(tmp_path: Path) -> ModuleType:
    ...
    bridge_path = tools_root / compat.C4_BRIDGE_PATH.name
    shutil.copy2(Path(compat.__file__), source_path)
    shutil.copy2(compat.FROZEN_ENVIRONMENT_PATH, frozen_path)
    shutil.copy2(compat.C4_BRIDGE_PATH, bridge_path)
    module_name = f"isolated_e4_{id(tmp_path)}"
...
def _prepare_isolated_cli(
    ...
    monkeypatch.setattr(
        module,
        "build_c4_postcleanup_in_memory",
...
def test_fixed_three_lane_writer_seals_exact_generations_in_tmp_repo(
    ...
    for flags, mode in create_calls:
        ...
        assert mode == 0o600
    lanes = (
        (module.C4_POLICY_PATH, "policy_fingerprint", payloads["policy"]),
        (
            module.C4_STABILITY_PATH,
            "stability_receipt_fingerprint",
            payloads["stability"],
        ),
        (
            module.C4_POSTCLEANUP_PATH,
            "receipt_fingerprint",
            payloads["postcleanup"],
        ),
    )
...
def test_fixed_writer_failure_is_preserved_and_never_retried(
    ...
    monkeypatch.setattr(module, "build_c4_policy_in_memory", build)
    ...
    assert module.C4_POLICY_PATH.exists()
    assert stat.S_IMODE(module.C4_POLICY_PATH.stat().st_mode) == 0o600
    assert module.C4_POLICY_PATH.stat().st_size == 0
...
def test_postcleanup_cross_roots_are_verified_while_output_fd_is_open(
    ...
    def output_fd_is_open() -> bool:
        for descriptor_path in Path("/proc/self/fd").iterdir():
            try:
                if descriptor_path.resolve() == module.C4_POSTCLEANUP_PATH:
                    return True
    ...
    def tracked_verify(path, evidence, fingerprint_field):
        ...
        events.append(
            (
                target,
                module.C4_POSTCLEANUP_PATH.exists(),
                output_fd_is_open(),
            )
        )
    ...
    guarded_while_open = [
        ...
        for event in events
        if event[0]
        in {module.C4_POLICY_PATH, module.C4_STABILITY_PATH}
        and event[1]
        and event[2]
    ]
    assert {event[0] for event in guarded_while_open} == {
        module.C4_POLICY_PATH,
        module.C4_STABILITY_PATH,
    }
    guarded_after_close = [
        event
    ...
    final_output_checks = [
        event for event in events if event[0] == module.C4_POSTCLEANUP_PATH
    ]
...
def test_postcleanup_guard_tamper_fails_closed_and_burns_attempt(
    ...
    def tamper_during_guard(path, evidence, fingerprint_field):
        ...
        target = Path(path).absolute()
        if (
            target == module.C4_POLICY_PATH
            and module.C4_POSTCLEANUP_PATH.exists()
            and not tampered
        ):
            for descriptor_path in Path("/proc/self/fd").iterdir():
                try:
                    ...
                    continue
            module.os.chmod(module.C4_POLICY_PATH, 0o600)
            tampered = True
        return real_verify(path, evidence, fingerprint_field)
    ...
    assert module.C4_POSTCLEANUP_PATH.exists()
    assert (
        stat.S_IMODE(module.C4_POSTCLEANUP_PATH.stat().st_mode)
        == 0o444
    )
    with pytest.raises(PermissionError, match="write order"):
        module.main(["postcleanup"])
...
def test_live_root_exact7_rejects_shape_and_mode_drift(
    ...
    _payload, root = module._read_live_sealed(
        module.C4_POLICY_PATH, "policy_fingerprint"
    )
    with pytest.raises(PermissionError):
    ...
    module.os.chmod(module.C4_POLICY_PATH, 0o400)
    with pytest.raises(PermissionError):
...
def _fixed_environment_values(monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    policy_path = tmp_path / "runtime_environment_policy_preaccess_compat_c4.json"
    stability_path = tmp_path / "runtime_environment_stability_receipt_preaccess_compat_c4.json"
    monkeypatch.setattr(compat, "C4_POLICY_PATH", policy_path)
    monkeypatch.setattr(compat, "C4_STABILITY_PATH", stability_path)
    monkeypatch.setattr(
        compat,
        "_LIVE_ROOT_SPECS",
        {
    ...
    old, c4, roots, archival, policy, stability = _policy_and_stability(monkeypatch)
    return old, c4, roots, archival, policy, stability


def _seal_fixed_environment_values(policy, stability):
    _seal_fixed_environment_payload(compat.C4_POLICY_PATH, policy)
    _loaded, policy_root = compat._read_live_sealed(
        compat.C4_POLICY_PATH, "policy_fingerprint"
    )
    ...
    _seal_fixed_environment_payload(compat.C4_STABILITY_PATH, closed)
    return closed
...
def _fixed_post(old, roots, archival, policy, stability):
    ...
    return compat.build_c4_postcleanup_in_memory(
        policy,
        stability,
        realization_validator=_validator(archival),
        prepare=_prepare(old, roots),
...
def test_closure_none_roots_auto_binds_fixed_policy_and_stability(
    ...
    old, c4, roots, archival, policy, stability = _fixed_environment_values(
    ...
    result = compat.validate_c4_environment_closure(
        policy, stability, post, archival=archival, c4_contract=c4
    )
    assert result["postcleanup"] == post
    assert observed == [compat.C4_POLICY_PATH, compat.C4_STABILITY_PATH]
...
def test_closure_none_roots_rejects_replaced_fixed_policy(
    ...
    stability = _seal_fixed_environment_values(policy, stability)
    post = _fixed_post(old, roots, archival, policy, stability)
    old_inode = compat.C4_POLICY_PATH.stat().st_ino
    ...
    os.replace(replacement, compat.C4_POLICY_PATH)
    assert compat.C4_POLICY_PATH.stat().st_ino != old_inode
    with pytest.raises(PermissionError):
        compat.validate_c4_environment_closure(
            policy, stability, post, archival=archival, c4_contract=c4
        )
...
def test_closure_none_roots_rejects_partial_fixed_state(
    ...
    stability = _seal_fixed_environment_values(policy, stability)
    post = _fixed_post(old, roots, archival, policy, stability)
    path = compat.C4_POLICY_PATH if missing == "policy" else compat.C4_STABILITY_PATH
    path.unlink()
...
def test_closure_none_roots_with_no_fixed_lanes_is_in_memory_only(
    ...
    post = _fixed_post(old, roots, archival, policy, stability)
    assert not compat.C4_POLICY_PATH.exists()
    assert not compat.C4_STABILITY_PATH.exists()
    result = compat.validate_c4_environment_closure(
        policy, stability, post, archival=archival, c4_contract=c4
    )
    assert result["postcleanup"] == post
...
def test_closure_explicit_policy_root_auto_upgrades_fixed_tt(
    ...
    _old, c4, _roots_value, archival, policy, stability = (
    ...
    assert observed[:2] == [
        compat.C4_POLICY_PATH, compat.C4_STABILITY_PATH
    ]
...
def test_closure_explicit_policy_root_cannot_skip_unsealed_fixed_stability(
    ...
    compat.C4_STABILITY_PATH.chmod(0o400)
    with pytest.raises(PermissionError):
        compat.validate_c4_environment_closure(
            policy,
            stability,
...
def test_fixed_writer_replays_contract_archival_and_closure_while_fd_open(
    ...
    payloads = _prepare_isolated_cli(module, monkeypatch)
    lane_paths = {
        "policy": module.C4_POLICY_PATH,
        "stability": module.C4_STABILITY_PATH,
        "postcleanup": module.C4_POSTCLEANUP_PATH,
    }
    lane_fields = {
    ...
    monkeypatch.setattr(module, "validate_c4_environment_closure", closure)
    monkeypatch.setattr(
        module,
        "_validate_c4_policy_contract",
        lambda policy, **_kwargs: dict(policy),
...
def test_future_command_cannot_fall_through_to_postcleanup_lane(
    ...
    module = _isolated_compatibility_module(tmp_path)
    payloads = _prepare_isolated_cli(module, monkeypatch)
    _seal_fixed_environment_payload(module.C4_POLICY_PATH, payloads["policy"])
    _seal_fixed_environment_payload(
        module.C4_STABILITY_PATH, payloads["stability"]
    )
    ...
    with pytest.raises(PermissionError):
        module.main([])
    assert not module.C4_POSTCLEANUP_PATH.exists()
...
def test_policy_writer_replays_contract_archival_and_policy_while_fd_open(
    ...
    module = _isolated_compatibility_module(tmp_path)
    _prepare_isolated_cli(module, monkeypatch)
    output_path = module.C4_POLICY_PATH
    events: list[tuple[str, bool]] = []
