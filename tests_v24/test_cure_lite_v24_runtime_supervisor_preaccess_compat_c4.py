...
from tools import (
    cure_lite_v24_preaccess_schema_compatibility_c4 as actual_bridge,
)
from tools import (
    cure_lite_v24_runtime_supervisor_preaccess_compat_c4 as supervisor,
)
...
def _actual_c4_spec(tmp_path: Path) -> dict[str, object]:
...
@pytest.mark.parametrize(
    ...
    [
        ("COMPAT_UNIT_NAME", supervisor._C3_UNIT_NAME),
        ("COMPAT_RUNTIME_SPEC_PATH", supervisor._C3_RUNTIME_SPEC_PATH),
        (
            "COMPAT_RUNTIME_LAUNCH_AUTHORIZATION_PATH",
            supervisor._C3_RUNTIME_LAUNCH_AUTHORIZATION_PATH,
        ),
        ("COMPAT_ADAPTER_PATH", supervisor._C3_ADAPTER_PATH),
        ("COMPAT_SUPERVISOR_PATH", supervisor._C3_SUPERVISOR_PATH),
        ("COMPATIBILITY_RECEIPT_PATH", supervisor._C3_RECEIPT_PATH),
        (
            "COMPAT_POLICY_SOURCE_PATH",
            Path(supervisor._C3_POLICY_SOURCE_PATH),
        ),
    ],
)
def test_no_c3_historical_name_can_become_an_active_c4_target(
    field: str,
    historical_value: object,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(supervisor, field, historical_value)
    with pytest.raises(PermissionError, match="c4 runtime identity changed"):
        supervisor._require_disjoint_c4_identity()
...
def test_fresh_interpreter_sentinel_source_fails_before_cli_dispatch() -> None:
    ...
    script = "\n".join(
        (
            ...
            "module = ModuleType('fresh_s4_sentinel')",
    ...
    assert completed.returncode != 0
    assert "c4 bridge source generation is not frozen" in completed.stderr


@pytest.mark.parametrize(
    "argv",
    [
        ["commit-and-start", "--spec", "/tmp/c4-guard-absent.json"],
        ["claim-materialization", "--spec", "/tmp/c4-guard-absent.json"],
        ["verify-runtime-spec", "--spec", "/tmp/c4-guard-absent.json"],
        ["run-once", "--spec", "/tmp/c4-guard-absent.json"],
        ["systemd-finalize", "--spec", "/tmp/c4-guard-absent.json"],
        ["record-systemd-exit", "--spec", "/tmp/c4-guard-absent.json"],
    ],
)
...
def test_raw_inherited_runtime_callables_are_guarded(
    ...
    with pytest.raises(
        ...
        inherited("/tmp/c4-guard-absent.json")
...
@pytest.mark.parametrize(
    "call",
    (
        lambda: supervisor.claim_materialization("/tmp/c4-guard-absent.json"),
        lambda: supervisor.verify_runtime_spec("/tmp/c4-guard-absent.json"),
        lambda: supervisor.run_once("/tmp/c4-guard-absent.json"),
        lambda: supervisor.finalize_systemd("/tmp/c4-guard-absent.json"),
        lambda: supervisor.load_runtime_spec("/tmp/c4-guard-absent.json"),
        lambda: supervisor.compat_c1.main(["--help"]),
        lambda: supervisor.compat_c1.commit_and_start(
            "/tmp/c4-guard-absent.json"
        ),
        ...
        lambda: supervisor.compat_c1.legacy.verify_child_runtime_attestation(
            "/tmp/c4-attestation-absent.json",
            "/tmp/c4-launch-absent.json",
        ),
    ),
)
...
def test_verified_identity_accepts_only_fixed_c4_runtime_names(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _enable_guarded_calls(monkeypatch)
    result = supervisor.verify_compatibility_identity()

    assert result["runtime_compatibility_generation"] == "c4"
    ...
    assert result["c4_supervisor_file_sha256"] == _sha256(
...
def test_extra_runtime_spec_field_is_rejected_before_bridge(
    ...
    _enable_guarded_calls(monkeypatch)
    payload = _actual_c4_spec(tmp_path)
    payload["unexpected_c4_field"] = "forbidden"
    _reseal_spec(payload)
    bridge_called = False
