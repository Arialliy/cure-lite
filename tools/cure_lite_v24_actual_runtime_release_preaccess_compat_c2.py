#!/usr/bin/env python3
"""Create-only runtime release wrapper for compatibility generation c2.
...
the disjoint c2 runtime namespace and adds three c2-specific consumers:

* the sealed c2 compatibility receipt (including its environment lineage);
* the fresh c2 environment scope cross-binding;
* the archival c2 unit-realization closure.
...
def _stable_source_bytes(path: Path) -> tuple[bytes, dict[str, int]]:
    ...
    if (
        ...
        raise PermissionError("c2 release source path is unsafe")
    ...
    try:
        ...
        if (
            ...
            raise PermissionError("c2 release source is unsafe")
        descriptor = os.open(
            target.name,
            os.O_RDONLY
            | getattr(os, "O_CLOEXEC", 0)
    ...
    if any(
        ...
        raise PermissionError("c2 release source generation changed")
    identity = {
...
def _load_verified(
    ...
    *,
    ...
    if hashlib.sha256(raw).hexdigest() != expected_sha256:
        raise PermissionError("bound c2 release component source changed")
    ...
    if raw_after != raw or after != identity:
        sys.modules.pop(name, None)
        raise PermissionError("bound c2 release component was replaced")
...
compat_bridge, _BRIDGE_LOAD_IDENTITY = _load_when_frozen(
    COMPAT_BRIDGE_PATH,
    COMPAT_BRIDGE_SHA256,
    name="tools._cure_lite_v24_preaccess_compat_c2_verified_for_release",
)
compat_realizer, _REALIZER_LOAD_IDENTITY = _load_when_frozen(
    COMPAT_REALIZER_PATH,
    COMPAT_REALIZER_SHA256,
    name="tools._cure_lite_v24_realizer_compat_c2_verified_for_release",
)
compat_supervisor, _SUPERVISOR_LOAD_IDENTITY = _load_when_frozen(
    COMPAT_SUPERVISOR_PATH,
    COMPAT_SUPERVISOR_SHA256,
    name="tools._cure_lite_v24_supervisor_compat_c2_verified_for_release",
)
compat_environment, _ENVIRONMENT_LOAD_IDENTITY = _load_when_frozen(
    COMPAT_ENVIRONMENT_PATH,
    COMPAT_ENVIRONMENT_SHA256,
    name="tools._cure_lite_v24_environment_compat_c2_verified_for_release",
)
...
def _require_component_sources() -> None:
    for path, digest, identity in (
        ...
        if (
            ...
            raise PermissionError("c2 release component hash is not frozen")
        raw, observed = _stable_source_bytes(path)
        if hashlib.sha256(raw).hexdigest() != digest or observed != identity:
            raise PermissionError("c2 release component generation changed")


def _require_lane_separation() -> None:
    if any(
        os.path.lexists(path)
        for path in (*BLOCKED_RUNTIME_PATHS, *C1_RUNTIME_PATHS)
    ):
        raise PermissionError(
            "direct or c1 predecessor runtime lane is no longer pristine"
        )
...
def _require_build_phase_pristine() -> None:
    if any(
        ...
        raise PermissionError("c2 build-spec namespace is already materialized")
...
def _require_prelaunch_scientific_pristine() -> None:
    if any(
        ...
        raise PermissionError(
            "scientific r2 output exists before c2 launch authorization"
        )
...
def _load_fixed_compat_runtime_spec() -> dict[str, object]:
    ...
    if not isinstance(payload, Mapping):
        raise PermissionError("fixed c2 runtime spec is not a mapping")
    return dict(payload)
...
def _verify_environment_cross_binding(
    ...
    old_contract, c2_contract, _roots = (
    ...
    archival = compat_environment.validate_c2_realization_archival(
        archival,
    ...
    closure = compat_environment.validate_c2_environment_closure(
        policy,
        stability,
        postcleanup,
        archival=archival,
    ...
    if (
        ...
        != compat_environment._normalized_contract(c2_contract)
        or not legacy._deep_exact_equal(
            closure.get("realization"), archival,
        )
    ):
        raise PermissionError(
            "c2 bridge/environment/realization cross-binding changed"
        )
    return closure
...
def _verify_compatibility_receipt(
    *,
    ...
    if (
        ...
        or result.get("runtime_compatibility_id") != "c2"
        ...
        raise PermissionError("c2 compatibility receipt is not exact")
...
def _verify_active_phase_compatibility_receipt() -> Mapping[str, object]:
    ...
    raise PermissionError("c2 release phase is not explicitly selected")
...
def _require_fixed_inputs(values: Mapping[str, object]) -> None:
    ...
    for name, expected_path in expected.items():
        ...
        if (
            ...
            raise PermissionError(f"c2 release input path changed: {name}")


def _require_final_r12_supervisor_binding(
    closure: Mapping[str, object],
) -> None:
    """Bind the archived r12 proof to this exact supervisor generation."""

    integration = closure.get("integration")
    if not isinstance(integration, Mapping):
        raise PermissionError(
            "r12 integration is not bound to the final c2 supervisor "
            "generation"
        )
...
def _compat_validate_release_closure(**kwargs):
    ...
    _require_final_r12_supervisor_binding(result)
    archival = compat_realizer.validate_archival_realization_chain(
        REALIZATION_AUTHORIZATION_PATH,
        REALIZATION_RECEIPT_PATH,
    )
    ...
    if (
        ...
        raise PermissionError("c2 archival release closure diverged")
    return result
...
_frozen_write_sealed = compat_c1._frozen_write_sealed
...
def _materialize_spec_body(
    ...
    *,
    ...
    if (
        ...
        raise PermissionError("c2 prewrite spec output contract changed")
    return {
        **dict(body),
...
def verify_compatibility_identity() -> dict[str, object]:
    ...
    if (
        ...
        raise PermissionError("c2 release identity changed")
    return {
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command not in _ALLOWED_COMMANDS:
        raise PermissionError("c2 release command is not allowed")
    ...
    try:
        ...
        if args.command == "build-spec":
            ...
            if not legacy._deep_exact_equal(preview, repeated):
                raise PermissionError("c2 build-spec preview is nondeterministic")
