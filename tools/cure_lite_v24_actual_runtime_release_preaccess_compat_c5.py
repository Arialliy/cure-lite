#!/usr/bin/env python3
"""Create-only runtime release wrapper for compatibility generation c5.
...
the disjoint c5 runtime namespace and adds three c5-specific consumers:

* the sealed c5 compatibility receipt (including its environment lineage);
* the fresh c5 environment scope cross-binding;
* the archival c5 unit-realization closure.
...
INTEGRATION_ROOT = (
    EVIDENCE_ROOT
    / "supervisor_v2_systemd_integration_preaccess_compat_c5_r14"
)
...
R14_SCENARIO_ID = (
    "supervisor-v2-dummy-compat-c5-r14-20260801c5000014"
)
R14_DUMMY_UNIT = (
    "cure-lite-v24-supervisor-integration-"
    "supervisor-v2-dummy-compat-c5-r14-20260801c5000014.service"
)
...
REALIZATION_TERMINAL_PATH = (
    EVIDENCE_ROOT
    / "r2_preaccess_compat_c5_unit_realization_terminal.json"
)
...
def _canonical_json(value: object) -> str:
    """Canonical profile owned by L5, never applied to foreign evidence."""
...
def _build_disjoint_c5_runtime_identity_gate():
    """Freeze the c5 runtime identity and its separation from older lanes."""
    ...
    expected_c4_r14_root = C4_R14_ROOT.absolute()
    expected_c4_protected_evidence = tuple(
        path.absolute() for path in C4_PROTECTED_EVIDENCE_PATHS
    )
    expected_aliases = (
    ...
    def require() -> None:
        ...
        c4_protected_evidence = tuple(
            Path(path).absolute() for path in C4_PROTECTED_EVIDENCE_PATHS
        )
        c5_control_paths = (
            COMPATIBILITY_RECEIPT_PATH.absolute(),
            POLICY_PATH.absolute(),
            SCOPE_HANDOFF_PATH.absolute(),
            STABILITY_ATTEMPT_PATH.absolute(),
            STABILITY_PATH.absolute(),
            POSTCLEANUP_PATH.absolute(),
            REALIZATION_AUTHORIZATION_PATH.absolute(),
            REALIZATION_RECEIPT_PATH.absolute(),
            REALIZATION_TERMINAL_PATH.absolute(),
            INTEGRATION_ROOT.absolute(),
        )
        if (
            COMPAT_UNIT != expected_unit
            or runtime_paths != expected_runtime_paths
            or predecessor_paths != expected_predecessor_paths
            or C4_R14_ROOT.absolute() != expected_c4_r14_root
            or c4_protected_evidence != expected_c4_protected_evidence
            ...
            or set(c4_protected_evidence).intersection(
                (*runtime_paths, *c5_control_paths)
            )
            or C4_R14_ROOT.absolute() == INTEGRATION_ROOT.absolute()
        ):
            raise PermissionError("c5 runtime identity/path isolation changed")

    return require


_require_disjoint_c5_runtime_identity = (
    _build_disjoint_c5_runtime_identity_gate()
)
...
def _stable_source_bytes(path: Path) -> tuple[bytes, dict[str, int]]:
    ...
    if (
        ...
        raise PermissionError("c5 release source path is unsafe")
    ...
    try:
        ...
        if (
            ...
            raise PermissionError("c5 release source is unsafe")
        descriptor = os.open(
            target.name,
            os.O_RDONLY
            | getattr(os, "O_CLOEXEC", 0)
    ...
    if any(
        ...
        raise PermissionError("c5 release source generation changed")
    identity = {
...
def _load_verified(
    ...
    *,
    ...
    if hashlib.sha256(raw).hexdigest() != expected_sha256:
        raise PermissionError("bound c5 release component source changed")
    ...
    if raw_after != raw or after != identity:
        sys.modules.pop(name, None)
        raise PermissionError("bound c5 release component was replaced")
...
_compat_c1, _FROZEN_RELEASE_LOAD_IDENTITY = _load_verified(
    FROZEN_RELEASE_PATH,
    FROZEN_RELEASE_SHA256,
    name=(
...
compat_bridge, _BRIDGE_LOAD_IDENTITY = _load_when_frozen(
    COMPAT_BRIDGE_PATH,
    COMPAT_BRIDGE_SHA256,
    name="tools._cure_lite_v24_preaccess_compat_c5_verified_for_release",
)
compat_realizer, _REALIZER_LOAD_IDENTITY = _load_when_frozen(
    COMPAT_REALIZER_PATH,
    COMPAT_REALIZER_SHA256,
    name="tools._cure_lite_v24_realizer_compat_c5_verified_for_release",
)
compat_supervisor, _SUPERVISOR_LOAD_IDENTITY = _load_when_frozen(
    COMPAT_SUPERVISOR_PATH,
    COMPAT_SUPERVISOR_SHA256,
    name="tools._cure_lite_v24_supervisor_compat_c5_verified_for_release",
)
compat_environment, _ENVIRONMENT_LOAD_IDENTITY = _load_when_frozen(
    COMPAT_ENVIRONMENT_PATH,
    COMPAT_ENVIRONMENT_SHA256,
    name="tools._cure_lite_v24_environment_compat_c5_verified_for_release",
)
...
_UNIT_TEMPLATE_LOAD_IDENTITY = _bind_when_frozen(
    COMPAT_UNIT_TEMPLATE_PATH,
    COMPAT_UNIT_TEMPLATE_SHA256,
)
...
_legacy = _compat_c1.legacy
_ACTIVE_COMMAND = _compat_c1._ACTIVE_COMMAND
_APPROVED_PREWRITE_SPEC = _compat_c1._APPROVED_PREWRITE_SPEC
_EXPECTED_SHADOW_READER = _legacy._default_shadow_reader
_EXPECTED_MANAGER_READER = (
    compat_realizer.legacy.collect_manager_generation
)
_EXPECTED_SUMMARY_RUNNER = _legacy._default_summary_runner
_RAW_LEGACY_MAIN_CODE = _legacy.main.__code__
_RAW_BUILD_SPEC_CODE = _legacy.build_spec.__code__
_RAW_AUTHORIZE_LAUNCH_CODE = _legacy.authorize_launch.__code__
_RAW_WRITE_SEALED_CODE = _compat_c1._frozen_write_sealed.__code__
_RAW_VALIDATE_RELEASE_CLOSURE_CODE = (
    _compat_c1._frozen_validate_release_closure.__code__
)
_RAW_CREATE_RUNTIME_DIRECTORIES_CODE = (
    _legacy._create_runtime_directories_and_verify_leaves.__code__
)
_C1_PREVIEW_BUILD_SPEC_CODE = _compat_c1._preview_build_spec.__code__
_frozen_validate_release_closure = (
    _compat_c1._frozen_validate_release_closure
)
...
def _load_l5_sealed(
    ...
    *,
    ...
    """Validate one L5-owned sealed object with the single UTF-8 profile."""
    ...
    if target not in {
        ...
        raise PermissionError("foreign evidence cannot use the L5 validator")
    raw, identity = _stable_source_bytes(target)
    if (
        identity["st_uid"] != os.getuid()
        or identity["st_nlink"] != 1
        or stat.S_IMODE(identity["st_mode"]) != 0o444
    ):
        raise PermissionError("L5 evidence is not exactly sealed")
    try:
        payload = json.loads(raw.decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError) as error:
        raise PermissionError("L5 evidence is not canonical JSON") from error
    if (
        not isinstance(payload, dict)
        or raw != (_canonical_json(payload) + "\n").encode("utf-8")
    ):
        raise PermissionError("L5 evidence is not canonical JSON")
    body = dict(payload)
    ...
    if (
        ...
        raise PermissionError("L5 evidence fingerprint/schema changed")
    if return_identity:
        return payload, {
            "path": str(target),
...
def _require_component_sources() -> None:
    for path, digest, identity in (
        ...
        if (
            ...
            raise PermissionError("c5 release component hash is not frozen")
        raw, observed = _stable_source_bytes(path)
        if hashlib.sha256(raw).hexdigest() != digest or observed != identity:
            raise PermissionError("c5 release component generation changed")


def _build_l5_delegation_gate():
    ...
    build_operations = frozenset(
        {
            "legacy-main",
            "build-spec",
            "validate-release-closure",
            "preview-build-spec",
            "create-runtime-directories",
            "frozen-create-runtime-directories",
            "private-directory",
            "compat-write-sealed",
            "frozen-write-sealed",
            "write-sealed-bound",
    ...
    authorize_operations = frozenset(
        {
            "legacy-main",
            "authorize-launch",
            "validate-release-closure",
    ...
    operations_by_command = {
        "build-spec": build_operations,
        "authorize-launch": authorize_operations,
    }
    state: ContextVar[
        tuple[object, str, tuple[str, ...], frozenset[str]] | None
    ] = ContextVar("cure_lite_v24_c5_release_delegation", default=None)
    operation_state: ContextVar[
        tuple[object, str, str, object] | None
    ] = ContextVar(
        "cure_lite_v24_c5_release_suboperation",
        default=None,
    )
    ...
    def require(
        ...
        *,
        ...
        expected_operations = operations_by_command.get(command)
        active_operation = operation_state.get()
        if (
            ...
            or capability[3] != expected_operations
            or (
                operation is not None
                and (
                    operation not in expected_operations
                    or operation_binding is None
                    or active_operation is None
                    or active_operation[0] is not seal
                    or active_operation[1] != command
                    or active_operation[2] != operation
                    or active_operation[3] is not operation_binding
                )
            )
            or (
                argv is not None
            ...
            raise PermissionError("c5 release delegation is not authorized")
    ...
    def enter_operation(
        ...
        delegated_argv = require(command)
        expected_operations = operations_by_command.get(command)
        if (
            expected_operations is None
            or operation not in expected_operations
        ):
            raise PermissionError("c5 release sub-operation is not authorized")
        token = operation_state.set(
            (seal, command, operation, operation_binding)
        )
        return delegated_argv, token

    def reset_operation(token: object) -> None:
        operation_state.reset(token)

    def require_no_operation() -> None:
        if operation_state.get() is not None:
            raise PermissionError("c5 release sub-operation capability leaked")

    def delegated_main(argv: Sequence[str] | None = None) -> int:
        materialized = list(sys.argv[1:] if argv is None else argv)
        _require_frozen_release_source()
        _require_component_sources()
        args = _legacy._parser().parse_args(materialized)
        if args.command not in _ALLOWED_COMMANDS:
            raise PermissionError("c5 release command is not allowed")
        ...
        delegation_token = state.set(
            (
                ...
                operations_by_command[args.command],
            )
        )
        operation_root_token = operation_state.set(None)
        preview_token = None
        try:
            ...
            if args.command == "build-spec":
                ...
                if not _legacy._deep_exact_equal(preview, repeated):
                    raise PermissionError(
                        "c5 build-spec preview is nondeterministic"
                    )
                preview_token = _APPROVED_PREWRITE_SPEC.set(preview)
            _require_frozen_release_source()
            _require_component_sources()
            return int(_legacy.main(materialized))
        finally:
            if preview_token is not None:
                _APPROVED_PREWRITE_SPEC.reset(preview_token)
            operation_state.reset(operation_root_token)
...
(
    main,
    _require_l5_delegation,
    _enter_l5_suboperation,
    _reset_l5_suboperation,
    _require_no_active_l5_suboperation,
) = _build_l5_delegation_gate()
del _build_l5_delegation_gate
_L5_DELEGATED_MAIN_CODE = main.__code__


def _validate_exact_write_suboperation(
    command: str,
    args: tuple[object, ...],
    kwargs: Mapping[str, object],
) -> None:
    if len(args) != 2 or set(kwargs) != {"fingerprint_field"}:
        raise PermissionError("c5 release write call shape changed")
    path, body = args
    fingerprint_field = kwargs["fingerprint_field"]
    if not isinstance(body, Mapping):
        raise PermissionError("c5 release write body is not a mapping")
    payload = {
        **dict(body),
        str(fingerprint_field): _stable_fingerprint(body),
    }
    if command == "build-spec":
        approved = _APPROVED_PREWRITE_SPEC.get()
        if (
            ...
            or not _legacy._deep_exact_equal(payload, dict(approved))
        ):
            raise PermissionError("c5 build-spec write binding changed")
        return
    if (
        command != "authorize-launch"
        or Path(path).absolute()
        != COMPAT_RUNTIME_LAUNCH_AUTHORIZATION_PATH.absolute()
        or fingerprint_field != "authorization_fingerprint"
    ):
        raise PermissionError("c5 authorize-launch write binding changed")
    spec, spec_identity = _load_l5_sealed(
        COMPAT_RUNTIME_SPEC_PATH,
        fingerprint_field="runtime_spec_fingerprint",
        schema=compat_supervisor.RUNTIME_SPEC_SCHEMA,
        return_identity=True,
    )
    validated = _legacy.validate_launch_authorization(
        payload,
        spec=spec,
        spec_identity=spec_identity,
    )
    if not _legacy._deep_exact_equal(validated, payload):
        raise PermissionError("c5 launch-authorization body changed")


def _validate_delegated_suboperation(
    operation: str,
    command: str,
    args: tuple[object, ...],
    kwargs: Mapping[str, object],
    *,
    delegated_argv: tuple[str, ...],
    exact_write_validator=_validate_exact_write_suboperation,
    expected_shadow_reader=_EXPECTED_SHADOW_READER,
    expected_manager_reader=_EXPECTED_MANAGER_READER,
    expected_summary_runner=_EXPECTED_SUMMARY_RUNNER,
) -> None:
    parsed = _legacy._parser().parse_args(list(delegated_argv))
    if parsed.command != command:
        raise PermissionError("c5 delegated command binding changed")
    if operation == "legacy-main":
        return
    if operation in {
        "compat-write-sealed",
        "frozen-write-sealed",
        ...
        exact_write_validator(command, args, kwargs)
        return
    if operation in {
        "create-runtime-directories",
        "frozen-create-runtime-directories",
        ...
        if (
            ...
            or not _legacy._deep_exact_equal(
                dict(args[0]),
                _legacy._artifact_paths(),
            )
        ):
            raise PermissionError("c5 runtime-directory operation changed")
        return
    if operation == "private-directory":
        if len(args) != 1 or set(kwargs) - {"create"}:
            raise PermissionError("c5 private-directory call shape changed")
        create = kwargs.get("create", False)
        if type(create) is not bool:
            raise PermissionError("c5 private-directory create flag changed")
        target = Path(args[0]).absolute()
        artifacts = _legacy._artifact_paths()
        build_create_targets = {
            COMPAT_RUNTIME_ARTIFACT_ROOT.absolute(),
            Path(artifacts["heartbeat_dir"]).absolute(),
            Path(artifacts["systemd_invocation_dir"]).absolute(),
            COMPAT_GPU_LEASE_ROOT.absolute(),
        }
        authorize_read_targets = {
            COMPAT_RUNTIME_ARTIFACT_ROOT.absolute(),
            Path(artifacts["heartbeat_dir"]).absolute(),
            Path(artifacts["systemd_invocation_dir"]).absolute(),
        }
        allowed = (
            target == EVIDENCE_ROOT.absolute() and create is False
        ) or (
            command == "build-spec"
            and create is True
            and target in build_create_targets
        ) or (
            command == "authorize-launch"
            and create is False
            and target in authorize_read_targets
        )
        if not allowed:
            raise PermissionError("c5 private-directory path binding changed")
        return
    if operation == "build-spec":
        expected_keys = {
            *_FIXED_INPUT_NAMES,
            "shadow_reader",
            "manager_reader",
        }
        if args or set(kwargs) != expected_keys:
            raise PermissionError("c5 build-spec call shape changed")
        _require_fixed_inputs(kwargs)
        if (
            kwargs.get("shadow_reader") is not expected_shadow_reader
            or kwargs.get("manager_reader")
            is not expected_manager_reader
        ):
            raise PermissionError("c5 build-spec callback binding changed")
        return
    if operation == "authorize-launch":
        if (
            args
            or set(kwargs)
            != {"shadow_reader", "manager_reader", "validity_seconds"}
            or kwargs.get("shadow_reader") is not expected_shadow_reader
            or kwargs.get("manager_reader")
            is not expected_manager_reader
            or kwargs.get("validity_seconds") != parsed.validity_seconds
            or type(kwargs.get("validity_seconds")) is not int
        ):
            raise PermissionError("c5 authorize-launch call binding changed")
        return
    if operation == "validate-release-closure":
        expected_keys = {
            *_FIXED_INPUT_NAMES,
            "shadow_reader",
            "manager_reader",
        ...
        if args or set(kwargs) != expected_keys:
            raise PermissionError("c5 release-closure call shape changed")
        _require_fixed_inputs(kwargs)
        if (
            kwargs.get("shadow_reader") is not expected_shadow_reader
            or kwargs.get("manager_reader")
            is not expected_manager_reader
            or kwargs.get("summary_runner")
            is not expected_summary_runner
        ):
            raise PermissionError("c5 release-closure callback binding changed")
        return
    if operation == "preview-build-spec":
        if len(args) != 1 or kwargs or getattr(args[0], "command", None) != command:
            raise PermissionError("c5 preview-build-spec call binding changed")
        _require_fixed_inputs(_fixed_build_values(args[0]))
        return
    raise PermissionError("c5 release sub-operation is not recognized")
...
def _guard_release_callable(
    ...
    *,
    operation: str,
    caller_codes: tuple[object, ...],
    bind_argv: bool = False,
    require_entry=_require_l5_delegation,
    validate_entry=_validate_delegated_suboperation,
    enter_entry=_enter_l5_suboperation,
    reset_entry=_reset_l5_suboperation,
):
    ...
    def guarded(*args, **kwargs):
        ...
        if selected not in _ALLOWED_COMMANDS:
            raise PermissionError("c5 release phase is not explicitly selected")
        ...
        validate_entry(
            ...
            args,
            kwargs,
            delegated_argv=capability_argv,
        )
        if sys._getframe(1).f_code not in caller_codes:
            raise PermissionError("c5 release delegated caller changed")
        operation_binding = object()
        _capability_argv, operation_token = enter_entry(
            selected,
            operation,
            operation_binding,
        )
        try:
            ...
            reset_entry(operation_token)

    guarded.__name__ = getattr(function, "__name__", "guarded_c5_release")
...
def _require_lane_separation() -> None:
    if any(
        ...
        raise PermissionError(
            "direct or c1-c4 predecessor runtime lane is no longer pristine"
        )
...
def _require_build_phase_pristine() -> None:
    if any(
        ...
        raise PermissionError("c5 build-spec namespace is already materialized")
...
def _require_prelaunch_scientific_pristine() -> None:
    if any(
        ...
        raise PermissionError(
            "scientific r2 output exists before c5 launch authorization"
        )


def _load_fixed_compat_runtime_spec() -> dict[str, object]:
    payload = _load_l5_sealed(
        COMPAT_RUNTIME_SPEC_PATH,
        fingerprint_field="runtime_spec_fingerprint",
        schema=compat_supervisor.RUNTIME_SPEC_SCHEMA,
    )
    if not isinstance(payload, Mapping):
        raise PermissionError("fixed c5 runtime spec is not a mapping")
    return dict(payload)


def _load_r5_archival() -> dict[str, object]:
    """Load R5 only through the frozen R5 producer validator."""
    ...
    if not isinstance(archival, Mapping):
        raise PermissionError("R5 archival validator returned no mapping")
    authorization = archival.get("authorization")
    receipt = archival.get("receipt")
    authorization_identity = archival.get("authorization_identity")
    receipt_identity = archival.get("receipt_identity")
    if (
        not isinstance(authorization, Mapping)
        or not isinstance(receipt, Mapping)
        or not isinstance(authorization_identity, Mapping)
        or not isinstance(receipt_identity, Mapping)
    ):
        raise PermissionError("R5 archival validator returned a partial chain")
    authorization_root = compat_environment._bind_r5_archival_root(
        REALIZATION_AUTHORIZATION_PATH,
        authorization_identity,
    )
    receipt_root = compat_environment._bind_r5_archival_root(
        REALIZATION_RECEIPT_PATH,
        receipt_identity,
    )
    compat_realizer._validate_c5_receipt_contract(receipt)
    if (
        ...
        raise PermissionError("R5 archival realization is not exact PASS")
    authorization_root_after = compat_environment._bind_r5_archival_root(
        REALIZATION_AUTHORIZATION_PATH,
        authorization_identity,
    )
    receipt_root_after = compat_environment._bind_r5_archival_root(
        REALIZATION_RECEIPT_PATH,
        receipt_identity,
    )
    if (
        not _legacy._deep_exact_equal(
            authorization_root,
            authorization_root_after,
        )
        or not _legacy._deep_exact_equal(receipt_root, receipt_root_after)
    ):
        raise PermissionError("R5 archival evidence generation changed")
    return {
        ...
        "compatibility_closure": dict(
            archival["compatibility_closure"]
        ),
    }
...
def _verify_environment_cross_binding(
    ...
    old_contract, c5_contract, _roots = (
        compat_environment.replay_old_scope_and_handoff()
    )
    archival = _load_r5_archival()
    archival = compat_environment.validate_c5_realization_archival(
        archival,
        contract=c5_contract,
    )
    policy, policy_root = compat_environment._read_live_sealed(
        POLICY_PATH,
        fingerprint_field="policy_fingerprint",
    )
    handoff, _handoff_root = compat_environment._read_live_sealed(
        SCOPE_HANDOFF_PATH,
        fingerprint_field="scope_handoff_fingerprint",
    )
    attempt, _attempt_root = compat_environment._read_live_sealed(
        STABILITY_ATTEMPT_PATH,
        fingerprint_field="stability_attempt_fingerprint",
    )
    stability, stability_root = compat_environment._read_live_sealed(
        STABILITY_PATH,
        fingerprint_field="stability_receipt_fingerprint",
    )
    postcleanup, _postcleanup_root = compat_environment._read_live_sealed(
        POSTCLEANUP_PATH,
        fingerprint_field="receipt_fingerprint",
    )
    closure = compat_environment.validate_c5_environment_closure(
        handoff,
        attempt,
        policy,
        stability,
    ...
    if (
        ...
        != compat_environment._normalized_contract(c5_contract)
        or not _legacy._deep_exact_equal(
            closure.get("realization"), archival,
        )
    ):
        raise PermissionError(
            "c5 bridge/environment/realization cross-binding changed"
        )
    return closure
...
def _verify_compatibility_receipt(
    *,
    ...
    if (
        ...
        or result.get("runtime_compatibility_id") != "c5"
        ...
        raise PermissionError("c5 compatibility receipt is not exact")
    expected_sources = {
        "compat_bridge": (COMPAT_BRIDGE_PATH, COMPAT_BRIDGE_SHA256),
        ...
        "compat_adapter": (COMPAT_ADAPTER_PATH, COMPAT_ADAPTER_SHA256),
    ...
    if not isinstance(roots, Mapping) or any(
        not isinstance(roots.get(label), Mapping)
        or roots[label].get("path") != str(path)
        or roots[label].get("file_sha256") != digest
        for label, (path, digest) in expected_sources.items()
    ):
        raise PermissionError("B5/L5 source-root closure changed")
...
def _require_fixed_inputs(values: Mapping[str, object]) -> None:
    ...
    for name, expected_path in expected.items():
        ...
        if (
            ...
            raise PermissionError(f"c5 release input path changed: {name}")
...
def _require_final_r14_supervisor_binding(
    ...
    if not isinstance(integration, Mapping):
        raise PermissionError(
            "r14 integration is not bound to the final c5 supervisor "
            "generation"
        )
...
def _validate_r14_archival_chronology(
    *,
    ...
    b5_time = _parse_archival_utc(
        b5_receipt.get("created_at_utc"),
        name="B5 receipt",
    )
...
def _snapshot_production_c5(
    *,
    ...
    if (
        ...
        raise PermissionError("production C5 static fragment changed")
    return {
        "archival": archival,
...
def _compat_validate_release_closure_impl(**kwargs):
    command = _ACTIVE_COMMAND.get()
    ...
    before = _snapshot_production_c5(**reader_kwargs)
    ...
    r14 = _validate_r14_runtime_chain(
        result["integration"],
        b5_receipt=bridge,
    )
    after = _snapshot_production_c5(**reader_kwargs)
    bridge_after = _verify_active_phase_compatibility_receipt()
    ...
    if isinstance(realization, Mapping):
        realization_authorization_root = (
            compat_environment._bind_r5_archival_root(
                REALIZATION_AUTHORIZATION_PATH,
    ...
    if (
        ...
        or not _legacy._deep_exact_equal(bridge, bridge_after)
        or not _legacy._deep_exact_equal(archival, after_archival)
        or not _legacy._deep_exact_equal(
        ...
        raise PermissionError("c5 archival release closure diverged")
    return result
...
def _compat_validate_release_closure(**kwargs):
    ...
    if command not in _ALLOWED_COMMANDS:
        raise PermissionError("c5 release phase is not explicitly selected")
    if sys._getframe(1).f_code not in {
        _RAW_BUILD_SPEC_CODE,
        _RAW_AUTHORIZE_LAUNCH_CODE,
    }:
        raise PermissionError("c5 release-closure caller changed")
    capability_argv = _require_l5_delegation(command)
    ...
    operation_binding = object()
    _capability_argv, operation_token = _enter_l5_suboperation(
        command,
        "validate-release-closure",
        operation_binding,
    )
    try:
        _require_l5_delegation(
            command,
            operation="validate-release-closure",
            operation_binding=operation_binding,
        )
        return _compat_validate_release_closure_impl(**kwargs)
    finally:
        _reset_l5_suboperation(operation_token)
...
def _configure() -> None:
    ...
    _require_component_sources()
    _require_disjoint_c5_runtime_identity()
    _compat_c1.FROZEN_RELEASE_PATH = FROZEN_RELEASE_PATH
    _compat_c1.FROZEN_RELEASE_SHA256 = FROZEN_RELEASE_SHA256
    _compat_c1._FROZEN_RELEASE_LOAD_IDENTITY = (
        _FROZEN_RELEASE_LOAD_IDENTITY
    )
    _compat_c1.COMPAT_REALIZER_PATH = COMPAT_REALIZER_PATH
    _compat_c1.COMPAT_REALIZER_SHA256 = COMPAT_REALIZER_SHA256
    _compat_c1.COMPAT_SUPERVISOR_PATH = COMPAT_SUPERVISOR_PATH
    _compat_c1.COMPAT_SUPERVISOR_SHA256 = COMPAT_SUPERVISOR_SHA256
    _compat_c1.compat_realizer = compat_realizer
    _compat_c1.compat_supervisor = compat_supervisor
    r14_integration.realizer = r14_shared_realizer
    _legacy.integration = r14_integration
    _compat_c1.COMPAT_UNIT = COMPAT_UNIT
    _compat_c1.COMPAT_RUNTIME_SPEC_PATH = COMPAT_RUNTIME_SPEC_PATH
    _compat_c1.COMPAT_RUNTIME_LAUNCH_AUTHORIZATION_PATH = (
        COMPAT_RUNTIME_LAUNCH_AUTHORIZATION_PATH
    )
    _compat_c1.COMPAT_RUNTIME_ARTIFACT_ROOT = COMPAT_RUNTIME_ARTIFACT_ROOT
    _compat_c1.COMPAT_GPU_LEASE_ROOT = COMPAT_GPU_LEASE_ROOT
    _compat_c1.COMPAT_RUN_ROOT_ALIAS_PATH = COMPAT_RUN_ROOT_ALIAS_PATH
    _compat_c1.COMPAT_RESULT_RECEIPT_ALIAS_PATH = (
        COMPAT_RESULT_RECEIPT_ALIAS_PATH
    )
    _compat_c1.SCIENTIFIC_RUN_ROOT = SCIENTIFIC_RUN_ROOT
    _compat_c1.SCIENTIFIC_RESULT_RECEIPT_PATH = (
        SCIENTIFIC_RESULT_RECEIPT_PATH
    )
    _compat_c1.COMPATIBILITY_RECEIPT_PATH = COMPATIBILITY_RECEIPT_PATH
    _compat_c1.POLICY_PATH = POLICY_PATH
    _compat_c1.STABILITY_PATH = STABILITY_PATH
    _compat_c1.POSTCLEANUP_PATH = POSTCLEANUP_PATH
    _compat_c1.INTEGRATION_AUTHORIZATION_PATH = (
        INTEGRATION_AUTHORIZATION_PATH
    )
    _compat_c1.INTEGRATION_RECEIPT_PATH = INTEGRATION_RECEIPT_PATH
    _compat_c1.REALIZATION_AUTHORIZATION_PATH = (
        REALIZATION_AUTHORIZATION_PATH
    )
    _compat_c1.REALIZATION_RECEIPT_PATH = REALIZATION_RECEIPT_PATH
    _compat_c1._require_lane_separation = _require_lane_separation
    _compat_c1._require_build_phase_pristine = (
        _require_build_phase_pristine
    )
    _compat_c1._require_prelaunch_scientific_pristine = (
        _require_prelaunch_scientific_pristine
    )
    _compat_c1._load_fixed_compat_runtime_spec = (
        _load_fixed_compat_runtime_spec
    )
    _compat_c1._verify_compatibility_receipt = (
        _verify_compatibility_receipt
    )
    _compat_c1._verify_active_phase_compatibility_receipt = (
        _verify_active_phase_compatibility_receipt
    )
    _compat_c1._require_fixed_inputs = _require_fixed_inputs
    _compat_c1._compat_validate_release_closure = (
        _compat_validate_release_closure
    )
    _compat_c1._materialize_spec_body = _materialize_spec_body
    _compat_c1._validate_prewrite_spec_body = _validate_prewrite_spec_body
    _compat_c1._compat_write_sealed = _compat_write_sealed
    _legacy.actual_realizer = compat_realizer.legacy
    _legacy.supervisor = compat_supervisor
    _legacy.canonical_json = _canonical_json
    _legacy.stable_fingerprint = _stable_fingerprint
    _legacy._load_sealed = _compat_load_sealed
    _legacy.validate_release_closure = _compat_validate_release_closure
    _legacy._write_sealed = _compat_write_sealed
    _legacy.UNIT_NAME = COMPAT_UNIT
    _legacy.SUPERVISOR_PATH = COMPAT_SUPERVISOR_PATH
    _legacy.ADAPTER_PATH = COMPAT_ADAPTER_PATH
    _legacy.RUNTIME_SPEC_PATH = COMPAT_RUNTIME_SPEC_PATH
    _legacy.RUNTIME_LAUNCH_AUTHORIZATION_PATH = (
        COMPAT_RUNTIME_LAUNCH_AUTHORIZATION_PATH
    )
    _legacy.RUNTIME_ARTIFACT_ROOT = COMPAT_RUNTIME_ARTIFACT_ROOT
    _legacy.GPU_LEASE_ROOT = COMPAT_GPU_LEASE_ROOT
...
def _materialize_spec_body(
    ...
    *,
    ...
    if (
        ...
        raise PermissionError("c5 prewrite spec output contract changed")
    return {
        **dict(body),
        fingerprint_field: _stable_fingerprint(body),
    }
...
def _compat_write_sealed(
    ...
    *,
    ...
    if sys._getframe(1).f_code is not _RAW_BUILD_SPEC_CODE:
        raise PermissionError("c5 spec-write caller changed")
    capability_argv = _require_l5_delegation("build-spec")
    _validate_delegated_suboperation(
        "compat-write-sealed",
        "build-spec",
        (path, body),
    ...
    try:
        ...
        if (
            ...
            or not _legacy._deep_exact_equal(payload, dict(preview))
        ):
            raise PermissionError(
...
def _preview_build_spec(args: object) -> dict[str, object]:
    if sys._getframe(1).f_code is not _L5_DELEGATED_MAIN_CODE:
        raise PermissionError("c5 preview caller changed")
    capability_argv = _require_l5_delegation("build-spec")
    _validate_delegated_suboperation(
        "preview-build-spec",
        "build-spec",
    ...
    try:
        _require_l5_delegation(
            "build-spec",
            operation="preview-build-spec",
            operation_binding=operation_binding,
        )
        return _compat_c1._preview_build_spec(args)
    finally:
        _reset_l5_suboperation(operation_token)


_frozen_write_sealed = _guard_release_callable(
    _compat_c1._frozen_write_sealed,
    "build-spec",
    operation="frozen-write-sealed",
    caller_codes=(_compat_write_sealed.__code__,),
)
...
def _compat_load_sealed(path: Path, **kwargs):
    ...
    if target in {
        ...
        allowed = {"fingerprint_field", "schema", "return_identity"}
        if set(kwargs) - allowed:
            raise PermissionError("L5 sealed-input options changed")
        return _load_l5_sealed(target, **kwargs)
    if (
        target == _legacy.SCIENTIFIC_ACCESS_AUDIT_PATH.absolute()
    ...
    return _compat_c1._frozen_load_sealed(target, **kwargs)


def _verify_compatibility_identity_impl() -> dict[str, object]:
    _configure()
    ...
    if (
        ...
        or _legacy.UNIT_NAME != COMPAT_UNIT
        or _legacy.RUNTIME_SPEC_PATH != COMPAT_RUNTIME_SPEC_PATH
    ):
        raise PermissionError("c5 release identity changed")
    return {
        ...
        "unit_template_path": str(COMPAT_UNIT_TEMPLATE_PATH),
        "unit_template_file_sha256": COMPAT_UNIT_TEMPLATE_SHA256,
        ...
        "protected_c4_runtime_paths": [
            str(path) for path in C4_RUNTIME_PATHS
        ],
        "protected_c4_r14_root": str(C4_R14_ROOT),
        "protected_c4_evidence_paths": [
            str(path) for path in C4_PROTECTED_EVIDENCE_PATHS
        ],
        "c4_mutation_authorized": False,
        "c4_reentry_authorized": False,
        "c4_r14_must_remain_absent": True,
        "scientific_run_root": str(SCIENTIFIC_RUN_ROOT),
        "scientific_result_receipt_path": str(
            SCIENTIFIC_RESULT_RECEIPT_PATH
        ),
        "allowed_splits": ["D_R"],
        "D_R_structural_update_count": 0,
...
def _install_inherited_release_guards() -> None:
    """Make every inherited mutation entry require one L5 main capability."""

    guarded_main = _guard_release_callable(
        _legacy.main,
        None,
        operation="legacy-main",
        caller_codes=(_L5_DELEGATED_MAIN_CODE,),
        bind_argv=True,
    )
    guarded_build = _guard_release_callable(
        _legacy.build_spec,
        "build-spec",
        operation="build-spec",
        caller_codes=(
            _RAW_LEGACY_MAIN_CODE,
            _C1_PREVIEW_BUILD_SPEC_CODE,
        ),
    )
    guarded_authorize = _guard_release_callable(
        _legacy.authorize_launch,
        "authorize-launch",
        operation="authorize-launch",
        caller_codes=(_RAW_LEGACY_MAIN_CODE,),
    )
    guarded_bound_writer = _guard_release_callable(
        _legacy._write_sealed_bound,
        None,
        operation="write-sealed-bound",
        caller_codes=(
            _RAW_WRITE_SEALED_CODE,
            _RAW_AUTHORIZE_LAUNCH_CODE,
        ),
    )
    guarded_private_directory = _guard_release_callable(
        _legacy._private_directory,
        None,
    ...
    guarded_runtime_directories = _guard_release_callable(
        _legacy._create_runtime_directories_and_verify_leaves,
        "build-spec",
        operation="create-runtime-directories",
        caller_codes=(_RAW_BUILD_SPEC_CODE,),
    )
    guarded_frozen_runtime_directories = _guard_release_callable(
        _compat_c1._frozen_create_runtime_directories,
        "build-spec",
        operation="frozen-create-runtime-directories",
        caller_codes=(),
    )

    def reject_c1_entrypoint(*_args, **_kwargs):
        raise PermissionError(
            "c1 release entrypoint is not a c5 delegation target"
        )

    _compat_c1.__dict__.pop("__getattr__", None)
    _legacy.main = guarded_main
    _legacy.build_spec = guarded_build
    _legacy.authorize_launch = guarded_authorize
    _legacy._write_sealed_bound = guarded_bound_writer
    _legacy._private_directory = guarded_private_directory
    _legacy._create_runtime_directories_and_verify_leaves = (
        guarded_runtime_directories
    )
    _legacy._write_sealed = _compat_write_sealed
    _legacy.validate_release_closure = _compat_validate_release_closure
    _compat_c1.main = reject_c1_entrypoint
    _compat_c1.verify_compatibility_identity = reject_c1_entrypoint
    _compat_c1._configure = reject_c1_entrypoint
    _compat_c1._frozen_write_sealed = _frozen_write_sealed
    _compat_c1._frozen_create_runtime_directories = (
        guarded_frozen_runtime_directories
    )
    _compat_c1._compat_write_sealed = _compat_write_sealed
...
class _GuardedLegacyView:
    """Read-only public projection of the inherited release producer."""
    ...
    def __getattr__(self, name: str):
        ...
        if name not in reads:
            raise PermissionError("c5 inherited release entry is private")
        value = reads[name]
        if isinstance(value, ModuleType):
            raise PermissionError("c5 inherited module is private")
        return value

    def __setattr__(self, _name: str, _value: object) -> None:
        raise PermissionError("c5 inherited release view is read-only")
...
class _ReadOnlyCompatC1View:
    """Read-only public projection of the frozen c1 wrapper."""
    ...
    def __getattr__(self, name: str):
        ...
        reads = {
            ...
            "COMPAT_UNIT": COMPAT_UNIT,
            "FROZEN_RELEASE_PATH": FROZEN_RELEASE_PATH,
            "FROZEN_RELEASE_SHA256": FROZEN_RELEASE_SHA256,
        }
        if name not in reads:
            raise PermissionError("c5 frozen c1 release entry is private")
        return reads[name]

    def __setattr__(self, _name: str, _value: object) -> None:
        raise PermissionError("c5 frozen c1 release view is read-only")
...
def _build_inherited_release_guard_identity_gate():
    module_entries = (
        (_legacy, "main", _legacy.main),
        (_legacy, "build_spec", _legacy.build_spec),
        (_legacy, "authorize_launch", _legacy.authorize_launch),
        (_legacy, "_write_sealed", _legacy._write_sealed),
        (_legacy, "_write_sealed_bound", _legacy._write_sealed_bound),
        (_legacy, "_private_directory", _legacy._private_directory),
        (
            _legacy,
        ...
        (
            ...
            _legacy.validate_release_closure,
        ),
        (_compat_c1, "main", _compat_c1.main),
        (
            _compat_c1,
            "verify_compatibility_identity",
            _compat_c1.verify_compatibility_identity,
        ),
        (_compat_c1, "_configure", _compat_c1._configure),
        (
            _compat_c1,
        ...
        (
            _compat_c1,
            "_compat_write_sealed",
            _compat_c1._compat_write_sealed,
        ),
    )
    global_entries = (
        ("main", main),
        ("_require_l5_delegation", _require_l5_delegation),
        ("_enter_l5_suboperation", _enter_l5_suboperation),
        ("_reset_l5_suboperation", _reset_l5_suboperation),
        (
            "_require_no_active_l5_suboperation",
            _require_no_active_l5_suboperation,
        ),
        (
            "_validate_exact_write_suboperation",
            _validate_exact_write_suboperation,
        ),
        (
            "_validate_delegated_suboperation",
        ...
        ("_guard_release_callable", _guard_release_callable),
        ("_FIXED_INPUT_NAMES", _FIXED_INPUT_NAMES),
        ("_EXPECTED_SHADOW_READER", _EXPECTED_SHADOW_READER),
        ("_EXPECTED_MANAGER_READER", _EXPECTED_MANAGER_READER),
        ("_EXPECTED_SUMMARY_RUNNER", _EXPECTED_SUMMARY_RUNNER),
        ("_frozen_write_sealed", _frozen_write_sealed),
        ("_compat_write_sealed", _compat_write_sealed),
        (
            "_compat_validate_release_closure",
            _compat_validate_release_closure,
        ),
    ...
    def require() -> None:
        if any(
            getattr(module, name, None) is not expected
            for module, name, expected in module_entries
        ) or any(
            globals().get(name) is not expected
            for name, expected in global_entries
        ):
            raise PermissionError("c5 inherited release guard identity changed")
        if (
            _compat_c1.legacy is not _legacy
            or _legacy._default_shadow_reader is not _EXPECTED_SHADOW_READER
            or _legacy._default_summary_runner is not _EXPECTED_SUMMARY_RUNNER
            or _legacy.actual_realizer.collect_manager_generation
            is not _EXPECTED_MANAGER_READER
        ):
            raise PermissionError("c5 inherited release callback identity changed")

    return require


_require_inherited_release_guard_identities = (
    _build_inherited_release_guard_identity_gate()
)
del _build_inherited_release_guard_identity_gate


def _build_public_identity_verifier(implementation, identity_guard):
    """Closure-bind the guard so rebinding its module name cannot bypass it."""
    ...
    return verify_compatibility_identity


verify_compatibility_identity = _build_public_identity_verifier(
    _verify_compatibility_identity_impl,
    _require_inherited_release_guard_identities,
)
del _build_public_identity_verifier
