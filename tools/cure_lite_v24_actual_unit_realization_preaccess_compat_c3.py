#!/usr/bin/env python3
"""Create-only unit realizer for runtime compatibility generation c3.
...
rebinds that verified implementation to the disjoint c3 unit namespace.

Only creation of the new c3 static fragment and ``daemon-reload`` are
authorized.  The original r2, c1, and consumed c2 units are immutable.  This
...
def _require_source_generations() -> None:
    ...
    if (
        ...
        raise PermissionError(
            "c3 realizer or frozen c1 predecessor generation changed",
        )
...
def _validate_c3_bridge_authorization(
    *,
    ...
    if (
        ...
        or policy.RUNTIME_COMPATIBILITY_ID != "c3"
    ):
        raise PermissionError("c3 bridge compatibility interface changed")
    ...
    if (
        ...
        raise PermissionError(
            "c3 bridge does not authorize the narrow unit lane",
        )
...
def _protected_units() -> dict[str, object]:
    _require_disjoint_c3_namespace()
    return {
...
def _require_disjoint_c3_namespace() -> None:
    units = (
        PROTECTED_ORIGINAL_UNIT,
        PROTECTED_C1_UNIT,
        PROTECTED_C2_UNIT,
    ...
    if (
        ...
        or "preaccess-compat-c3" not in COMPAT_UNIT
        or any("compat_c3" not in path.name for path in evidence)
    ):
        raise PermissionError("c3 unit/evidence namespace is not disjoint")


def _build_c3_closure(
    ...
    closure["runtime_compatibility_generation"] = "c3"
...
def _validate_c3_closure(
    ...
    *,
    ...
    if (
        ...
        raise PermissionError("c3 compatibility closure identity changed")
    body = dict(closure)
    fingerprint = body.pop("closure_fingerprint", None)
    if fingerprint != compat_c1._fingerprint(body):
        raise PermissionError("c3 compatibility closure fingerprint changed")
...
def _configure_c3_identity() -> None:
    ...
    compat_c1._build_compatibility_closure = _build_c3_closure
    compat_c1._validate_compatibility_closure = _validate_c3_closure
...
def verify_compatibility_identity() -> dict[str, object]:
    ...
    if COMPAT_BRIDGE_SOURCE_SHA256 == "__TO_BE_FROZEN__":
        raise PermissionError("c3 bridge source generation is not frozen")
    if COMPAT_TEMPLATE_SHA256 == "__TO_BE_FROZEN__":
        raise PermissionError("c3 template generation is not frozen")
    ...
    if any(result.get(key) != value for key, value in expected.items()):
        raise PermissionError("c3 realizer transitive identity changed")
    result.update(
        {
            "runtime_compatibility_generation": "c3",
            "frozen_c1_realizer_generation": predecessor,
            "compat_c3_realizer_generation": own,
...
class _GuardedLegacyView:
    """Read-compatible view with every write lane routed through c3."""
