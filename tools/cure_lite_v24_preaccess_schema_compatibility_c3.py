#!/usr/bin/env python3
"""Append-only closure for CURE-Lite v24 runtime compatibility generation c3.

This module is deliberately not a scientific runner.  It can only authorize
and seal the metadata-only c3 compatibility lane after the consumed c2
authorization-mode failure has been terminalized.  It never imports torch,
opens a dataset, starts a unit, or creates a runtime specification.

The c3 lane retains scientific attempt ordinal 2 and the original scientific
output paths.  All old, c1, c2, and scientific runtime paths remain protected.
...
RECEIPT_SCHEMA = (
    "cure-lite-v24-r2-preaccess-schema-compat-c3-receipt-v1"
)
...
# Downstream c3 realizer/supervisor/release modules must not need a permissive
# fallback or generation-specific import shim.
COMPAT_AUTHORIZATION_PATH = C3_AUTHORIZATION_PATH
COMPAT_RECEIPT_PATH = C3_RECEIPT_PATH
COMPATIBILITY_RECEIPT_PATH = C3_RECEIPT_PATH
COMPAT_UNIT_REALIZER_SOURCE_PATH = C3_UNIT_REALIZER_SOURCE_PATH
COMPAT_UNIT_NAME = C3_UNIT_NAME
...
def _expected_scientific_output_contract() -> dict[str, object]:
    return {
        ...
        "compat_run_root_alias": str(C3_RUN_ROOT_ALIAS_PATH),
...
def _expected_mutation_authority() -> dict[str, object]:
    return {
        ...
        "c3_unit_realization_authorized": True,
...
def _source_paths() -> dict[str, Path]:
    return {
        "compat_bridge": C3_BRIDGE_SOURCE_PATH,
        "c1_failure_terminalizer": (
            C1_FAILURE_TERMINALIZER_SOURCE_PATH
        ),
        "compat_environment_wrapper": (
        ...
        "compat_release": C3_RELEASE_SOURCE_PATH,
        "compat_supervisor": C3_SUPERVISOR_SOURCE_PATH,
        "compat_adapter": C3_ADAPTER_SOURCE_PATH,
        "compat_unit_realizer": C3_UNIT_REALIZER_SOURCE_PATH,
        "compat_unit_template": C3_UNIT_TEMPLATE_PATH,
        "c2_mode_contract_failure_terminalizer": (
            C2_MODE_CONTRACT_FAILURE_TERMINALIZER_SOURCE_PATH
        ),
        "c2_prewrite_failure_terminalizer": (
...
def _evidence_paths() -> dict[str, Path]:
    return {
        ...
        "environment_policy": C3_ENVIRONMENT_POLICY_PATH,
        "environment_stability": C3_ENVIRONMENT_STABILITY_PATH,
        "environment_postcleanup": C3_ENVIRONMENT_POSTCLEANUP_PATH,
        "unit_realization_authorization": C3_UNIT_AUTHORIZATION_PATH,
        "unit_realization_receipt": C3_UNIT_RECEIPT_PATH,
    }
...
def _collect_source_roots() -> dict[str, dict[str, object]]:
    ...
    if set(roots) != _SOURCE_LABELS:
        raise AssertionError("c3 source labels changed")
    if (
...
def _validate_source_roots(roots: object) -> None:
    if not isinstance(roots, Mapping) or set(roots) != _SOURCE_LABELS:
        raise PermissionError("c3 source-root labels changed")
    for label, path in _source_paths().items():
        expected = None
...
def _c3_preauthorization_paths() -> dict[str, Path]:
    return {
        "c3_environment_policy": C3_ENVIRONMENT_POLICY_PATH,
        "c3_environment_stability": C3_ENVIRONMENT_STABILITY_PATH,
        "c3_environment_postcleanup": C3_ENVIRONMENT_POSTCLEANUP_PATH,
        "c3_unit_authorization": C3_UNIT_AUTHORIZATION_PATH,
        "c3_unit_receipt": C3_UNIT_RECEIPT_PATH,
        "c3_unit_terminal": C3_UNIT_TERMINAL_PATH,
        "c3_unit_fragment": C3_UNIT_FRAGMENT_PATH,
    }


def _c3_future_paths() -> dict[str, Path]:
    return {
...
def _validate_scientific_output_phase(
    *,
    ...
    if phase != RUNTIME_PHASE_FINALIZE_SUCCESS:
        raise AssertionError("unhandled c3 runtime phase")
    _validate_runtime_scientific_run_root(require_empty=False)
...
def _validate_common_identity(value: Mapping[str, object]) -> None:
    if (
        ...
        raise PermissionError("c3 scientific identity changed")
...
def _load_verified_environment_wrapper(
    ...
    """Load exactly the wrapper generation captured by c3 authorization.

    The wrapper is intentionally not hash-pinned in this bridge: doing so
    would form a source-hash cycle once the wrapper pins the c3 realizer.  The
    ...
    if not isinstance(expected_root, Mapping):
        raise PermissionError("c3 environment wrapper root is malformed")
    expected = dict(expected_root)
    _validate_source_root(
        expected,
        expected_path=C3_ENVIRONMENT_WRAPPER_SOURCE_PATH,
    )
    raw, _observed = _read_regular_bytes(
        C3_ENVIRONMENT_WRAPPER_SOURCE_PATH,
        sealed=False,
    ...
    if hashlib.sha256(raw).hexdigest() != expected.get("file_sha256"):
        raise PermissionError("c3 environment wrapper bytes changed")
    name = "tools._cure_lite_v24_environment_compat_c3_verified_for_bridge"
    module = ModuleType(name)
    module.__file__ = str(C3_ENVIRONMENT_WRAPPER_SOURCE_PATH)
    module.__package__ = "tools"
    ...
    try:
        exec(
            compile(
                raw,
                str(C3_ENVIRONMENT_WRAPPER_SOURCE_PATH),
                "exec",
                dont_inherit=True,
            ),
            module.__dict__,
        )
        after = _source_root(C3_ENVIRONMENT_WRAPPER_SOURCE_PATH)
        if after != expected:
            raise PermissionError(
                "c3 environment wrapper generation changed while loading"
            )
        if (
            Path(module.C3_POLICY_PATH).absolute()
            != C3_ENVIRONMENT_POLICY_PATH.absolute()
            or Path(module.C3_STABILITY_PATH).absolute()
            != C3_ENVIRONMENT_STABILITY_PATH.absolute()
            or Path(module.C3_POSTCLEANUP_PATH).absolute()
            != C3_ENVIRONMENT_POSTCLEANUP_PATH.absolute()
            or Path(module.C3_REALIZATION_AUTHORIZATION_PATH).absolute()
            != C3_UNIT_AUTHORIZATION_PATH.absolute()
            or Path(module.C3_REALIZATION_RECEIPT_PATH).absolute()
            != C3_UNIT_RECEIPT_PATH.absolute()
            or module.C3_TARGET_UNIT != C3_UNIT_NAME
            or not callable(module.replay_old_scope_and_handoff)
            or not callable(module.validate_c3_environment_closure)
            or not callable(module._production_archival_validator)
        ):
            raise PermissionError("c3 environment wrapper interface changed")
...
def _normalize_environment_contract(contract: object) -> dict[str, object]:
    if not is_dataclass(contract) or isinstance(contract, type):
        raise PermissionError("c3 environment contract is not a dataclass")
    value = asdict(contract)
    if not isinstance(value, dict):
        raise PermissionError("c3 environment contract is malformed")
...
def _load_verified_mode_contract_failure_terminalizer(
    ...
    name = "tools._cure_lite_v24_c2_mode_contract_terminal_verified_for_c3_bridge"
    module = ModuleType(name)
...
def _validate_mode_contract_failure_terminal(
    ...
    # F's payload and historical-live claims are not C3 authority.  Only the
    # fixed source/evidence lineage and the C2 -> C3 transition are consumed.
...
def _authorization_times(
    ...
    *,
    ...
    created = _parse_utc(
        ...
        name="c3 authorization creation",
    )
    issued = _parse_utc(
        authorization.get("issued_at_utc"),
        name="c3 authorization issuance",
    )
    expires = _parse_utc(
        authorization.get("expires_at_utc"),
        name="c3 authorization expiry",
    )
    if (
        ...
        raise PermissionError("c3 authorization is stale or malformed")
...
def authorize_c3(
    *,
    ...
    if authorization_path is None:
        authorization_path = C3_AUTHORIZATION_PATH
    _require_fixed_path(
        authorization_path,
        C3_AUTHORIZATION_PATH,
        name="c3 authorization",
    )
    if (
        ...
        raise ValueError("c3 authorization input changed")
    if os.path.lexists(C3_AUTHORIZATION_PATH) or os.path.lexists(
        C3_RECEIPT_PATH
    ):
        raise FileExistsError("c3 compatibility identity is consumed")
    _validate_scientific_output_phase(
        allow_runtime_activation=False,
        runtime_phase=RUNTIME_PHASE_PREACTIVATION,
    )
    _require_absent(_c3_preauthorization_paths())
    _require_absent(_c3_future_paths())
    ...
    if issued <= c1_expires or issued <= c2_failure_sealed:
        raise PermissionError("c3 cannot precede its sealed predecessors")
...
def validate_c3_authorization(
    ...
    *,
    ...
    if set(authorization) != _AUTHORIZATION_KEYS:
        raise PermissionError("c3 authorization keys changed")
    _validate_scientific_output_phase(
        allow_runtime_activation=allow_runtime_activation,
        runtime_phase=phase,
    )
    ...
    if authorization.get("r10_roots") != _r10_roots_from_terminal(
        ...
        raise PermissionError("c3 authorization r10 lineage changed")
    ...
    failure_sealed = _parse_utc(
        _failure["identity"]["sealed_at_utc"],
        name="c2 mode-contract failure sealing",
    )
    if (
        authorized_at <= failure_sealed
        or authorization.get("runtime_compatibility_id") != "c3"
        ...
        != {"old": old_state, "c1": c1_state, "c2": c2_state}
        ...
        raise PermissionError("c3 authorization closure changed")
...
def validate_compat_authorization(
    ...
    *,
    ...
    """c1-compatible consumer interface for the c3 authorization.

    ``require_future_absence`` controls only the c3 runtime spec/launch/
    ...
    authorization, root = validate_c3_authorization(
        path=path,
        unit_state_reader=unit_state_reader,
        require_fresh=require_fresh,
        allow_runtime_activation=False,
...
def _validate_unit_chain(
    *,
    ...
    issued = _parse_utc(
        authorization.get("issued_at_utc"),
        name="c3 realization issuance",
    )
    expires = _parse_utc(
        authorization.get("expires_at_utc"),
        name="c3 realization expiry",
    )
    receipt_time = _parse_utc(
        receipt.get("created_at_utc"),
        name="c3 realization receipt creation",
    )
    full_shadow = receipt.get("full_static_shadow")
    fragment = receipt.get("fragment_identity")
    if (
        authorization.get("unit_name") != C3_UNIT_NAME
        or receipt.get("unit_name") != C3_UNIT_NAME
        ...
        or full_shadow.get("Id") != C3_UNIT_NAME
        or full_shadow.get("FragmentPath") != fragment.get("path")
        or receipt.get("manager_generation")
        != authorization.get("manager_generation")
    ):
        raise PermissionError("c3 unit realization chain changed")
    live = _normalized_state(unit_state_reader, C3_UNIT_NAME)
    if live.get("FragmentPath") != fragment.get("path"):
        raise PermissionError("c3 fragment path changed")
    ...
    if (
        ...
        raise PermissionError("c3 fragment generation changed")
    for field in _STATE_FIELDS:
        if field in full_shadow and live.get(field) != full_shadow[field]:
            if (
                ...
                continue
            raise PermissionError(f"c3 live unit shadow changed: {field}")
...
def _collect_full_closure(
    *,
    ...
    _require_inert_state(c1_state, unit_name=C1_UNIT_NAME)
    _require_missing_inert_state(c2_state, unit_name=C2_UNIT_NAME)
    if authorization.get("protected_unit_states") != {
        "old": old_state,
        "c1": c1_state,
        "c2": c2_state,
    }:
    ...
    if not isinstance(sources, Mapping):
        raise PermissionError("c3 source roots are malformed")
    if (
    ...
    if wrapper_root != dict(sources["compat_environment_wrapper"]):
        raise PermissionError("c3 environment wrapper generation changed")
    (
        unit_authorization,
        unit_auth_root,
        unit_receipt,
    ...
    if (
        ...
        raise PermissionError("c3 realization archival validator diverged")
    ...
    validated = environment.validate_c3_environment_closure(
        policy,
        stability,
        postcleanup,
        archival=archival,
        c3_contract=current_contract,
    )
    if (
        ...
        raise PermissionError("c3 environment wrapper returned a different closure")
    policy_time = _parse_utc(
        policy.get("created_at_utc"),
        name="c3 environment policy creation",
    )
    postcleanup_time = _parse_utc(
        postcleanup.get("created_at_utc"),
        name="c3 postcleanup creation",
    )
    ...
    if (
        ...
        raise PermissionError("c3 closure chronology/lineage changed")
    evidence_roots = {
    ...
    if set(evidence_roots) != _EVIDENCE_LABELS:
        raise AssertionError("c3 evidence labels changed")
    return {
...
def seal_receipt(
    *,
    ...
    if receipt_path is None:
        receipt_path = C3_RECEIPT_PATH
    _require_fixed_path(
        receipt_path,
        C3_RECEIPT_PATH,
        name="c3 receipt",
    )
    if os.path.lexists(C3_RECEIPT_PATH):
        raise FileExistsError("c3 compatibility receipt already exists")
    _validate_scientific_output_phase(
        allow_runtime_activation=False,
        runtime_phase=RUNTIME_PHASE_PREACTIVATION,
    )
    ...
    authorization, authorization_root = validate_c3_authorization(
        unit_state_reader=unit_state_reader,
        require_fresh=True,
        allow_runtime_activation=False,
        runtime_phase=RUNTIME_PHASE_PREACTIVATION,
    ...
    if not issued <= created <= expires:
        raise PermissionError("c3 receipt is outside authorization window")
    body: dict[str, object] = {
...
def _validate_receipt_evidence_roots(
    ...
    if (
        ...
        raise PermissionError("c3 receipt evidence-root labels changed")
...
def _validate_expected_runtime_spec_contract(
    ...
    if (
        ...
        != SCIENTIFIC_ATTEMPT_ORDINAL
        or systemd.get("unit_name") != C3_UNIT_NAME
        or artifacts.get("root") != str(C3_RUNTIME_ARTIFACT_ROOT)
    ):
        raise PermissionError("expected spec is outside c3")
...
def verify_compatibility_receipt(
    ...
    *,
    ...
    if type(require_spec_binding) is not bool:
        raise TypeError("c3 verification phase flags must be boolean")
    if (
        (expected_spec is None and require_spec_binding)
        or (expected_spec is not None and not require_spec_binding)
    ):
        raise PermissionError("c3 runtime-spec verification phase changed")
    receipt, receipt_root = _load_sealed(
        fixed,
        fingerprint_field="receipt_fingerprint",
    ...
    if set(receipt) != _RECEIPT_KEYS:
        raise PermissionError("c3 receipt keys changed")
    if (
        receipt.get("runtime_compatibility_id") != "c3"
        ...
        raise PermissionError("c3 receipt semantics changed")
    authorization, authorization_root = validate_c3_authorization(
        unit_state_reader=unit_state_reader,
        require_fresh=False,
        allow_runtime_activation=allow_runtime_activation,
    ...
    if not issued <= receipt_time <= expires:
        raise PermissionError("archival c3 receipt chronology changed")
    closure = _collect_full_closure(
        authorization=authorization,
        authorization_root=authorization_root,
        unit_state_reader=unit_state_reader,
    ...
    if (
        ...
        raise PermissionError("c3 compatibility receipt lineage changed")
    _validate_scientific_output_phase(
        allow_runtime_activation=allow_runtime_activation,
        runtime_phase=phase,
    )
    if expected_spec is None:
        ...
        if not isinstance(expected_spec, Mapping):
            raise TypeError("expected c3 runtime spec must be a mapping")
        ...
        if sealed_spec != dict(expected_spec):
            raise PermissionError("expected c3 runtime spec changed")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="CURE-Lite v24 c3 preaccess compatibility closure",
    )
    subparsers = parser.add_subparsers(dest="command", required=True)
    authorize = subparsers.add_parser("authorize-c3")
