...
C4_BRIDGE_SHA256 = (
    "cdbbe4355b29519d2b3da858732bc8531396a59f5a3f1cfacdb578323fe43de1"
)
...
def run_c4_stability_in_memory(
    ...
    *,
    ...
    """Run the frozen c4 gate and preserve any supplied live policy root."""
    ...
    _old, c4_contract, roots = replay_old_scope_and_handoff(
        prepare=prepare,
        activation_guard_reader=activation_guard_reader,
    )
    archival = _resolve_archival(
...
def validate_c4_environment_closure(
    ...
    *,
    ...
    if not realization_time < policy_time:
        raise PermissionError("c4 realization/policy chronology changed")
...
def _bind_private_cli() -> Callable[[Sequence[str] | None], int]:
    ...
    def bound_main(argv: Sequence[str] | None = None) -> int:
        ...
        if args.command == "stability-gate":
            ...
            stability = run_c4_stability_in_memory(
                policy,
                realization_validator=validator,
                live_roots={"policy": policy_root},
            )
