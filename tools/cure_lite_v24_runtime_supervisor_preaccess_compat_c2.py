#!/usr/bin/env python3
"""Runtime-supervisor wrapper for v24 compatibility generation c2.
...
identities, and puts a c2 source-generation guard in front of every inherited
...
only identity boundary that remains usable while the c2 bridge hash is still a
...
if str(REPOSITORY) not in sys.path:
    ...
    # repository root needed by the sealed c2 bridge import.
...
# This value is replaced only after the c2 bridge and its tests are frozen.
...
def _require_source_generations() -> None:
    """Require the exact loaded self, c1, and frozen c2 bridge generations."""
    ...
    if _BRIDGE_LOAD is None:
        raise PermissionError(
            "c2 bridge source was not frozen at supervisor load"
        )
    ...
    if (
        ...
        raise PermissionError(
            "c2 bridge source generation changed after supervisor load"
        )


def _require_disjoint_c2_identity() -> None:
    expected = {
        "unit": (
            "cure-lite-v24-gcr-pacre-dr-r2-preaccess-compat-c2.service"
        ),
        ...
        "policy": str(
            (
                REPOSITORY
                / "tools/"
                "cure_lite_v24_preaccess_schema_compatibility_c2.py"
            ).resolve()
        ),
    }
    c2_values = {
    ...
    if (
        ...
        raise PermissionError("compatibility c2 runtime identity changed")


def _configure_c2_identity() -> None:
    _require_disjoint_c2_identity()
...
_ACTIVE_RUNTIME_PHASE: ContextVar[str] = ContextVar(
    "cure_lite_v24_c2_runtime_phase",
    default="preactivation",
)
_RAW_LEGACY_RUNTIME_ENTRYPOINTS = {
...
def verify_compatibility_identity() -> dict[str, object]:
    ...
    if any(result.get(key) != value for key, value in expected.items()):
        raise PermissionError("compatibility c2 supervisor identity changed")
    forbidden_values = {
        _C1_UNIT_NAME,
        _OLD_UNIT_NAME,
        _C1_RUNTIME_SPEC_PATH,
    ...
    if any(
        ...
        raise PermissionError(
            "compatibility c2 identity retained an old runtime lane"
        )
    result.update(
        {
            "compatibility_policy_source_path": str(
            ...
            "c2_supervisor_file_sha256": _SELF_LOAD_SHA256,
...
def _verify_policy_compatibility_receipt(
    ...
    if runtime_phase not in _RUNTIME_PHASES:
        raise PermissionError("compatibility c2 runtime phase is invalid")
...
def _phase_guarded_inherited_callable(
    ...
    def guarded(*args, **kwargs):
        ...
        if selected not in _RUNTIME_PHASES - {"preactivation"}:
            raise PermissionError(
                "compatibility c2 runtime phase is invalid"
            )
...
def _install_inherited_runtime_guards() -> None:
    ...
    for name, function in _RAW_LEGACY_RUNTIME_ENTRYPOINTS.items():
        guarded = (
            _guarded_inherited_callable(function)
            if name == "main"
            else _phase_guarded_inherited_callable(
                function,
                phases[name],
            )
        )
...
# ensures calls reached through the frozen supervisor also cross the c2 guard.
...
def __getattr__(name: str):
    value = getattr(compat_c1, name)
    if callable(value):
        # Pure metadata callers have the explicit
        # describe_compatibility_identity boundary.  Any other inherited
        # callable is treated as a runtime entry point and guarded.
        return _guarded_inherited_callable(value)
    return value
