#!/usr/bin/env python3
"""Runtime-supervisor wrapper for v24 compatibility generation c4.
...
identities, and puts a c4 source-generation guard in front of every inherited
...
only identity boundary that remains usable while the c4 bridge hash is still a
...
if str(REPOSITORY) not in sys.path:
    ...
    # repository root needed by the sealed c4 bridge import.
...
COMPATIBILITY_RECEIPT_PATH = str(
    (
        REPOSITORY
        / "protocols/IRSTD-1K/gcr_pacre_v24/runtime_evidence_r2/"
        "r2_preaccess_schema_compat_c4_receipt.json"
    ).resolve()
)
...
# This value is replaced only after the c4 bridge and its tests are frozen.
...
def _require_source_generations() -> None:
    """Require the exact loaded self, c1, and frozen c4 bridge generations."""

    _require_self_and_c1_generations()
    if _SHA256.fullmatch(COMPAT_POLICY_SOURCE_SHA256) is None:
        raise PermissionError("c4 bridge source generation is not frozen")
    if _BRIDGE_LOAD is None:
        raise PermissionError(
            "c4 bridge source was not frozen at supervisor load"
        )
    ...
    if (
        ...
        raise PermissionError(
            "c4 bridge source generation changed after supervisor load"
        )


def _require_disjoint_c4_identity() -> None:
    expected = {
        "unit": (
            "cure-lite-v24-gcr-pacre-dr-r2-preaccess-compat-c4.service"
        ),
    ...
    if (
        ...
        raise PermissionError("compatibility c4 runtime identity changed")


def _configure_c4_identity() -> None:
    _require_disjoint_c4_identity()
...
def _guarded_inherited_callable(
    ...
    guarded.__name__ = getattr(function, "__name__", "guarded_c4_callable")
...
_ACTIVE_RUNTIME_PHASE: ContextVar[str] = ContextVar(
    "cure_lite_v24_c4_runtime_phase",
...
def verify_compatibility_identity() -> dict[str, object]:
    ...
    if any(result.get(key) != value for key, value in expected.items()):
        raise PermissionError("compatibility c4 supervisor identity changed")
    forbidden_values = {
        _C1_UNIT_NAME,
        _OLD_UNIT_NAME,
        _C2_UNIT_NAME,
    ...
    if any(
        ...
        raise PermissionError(
            "compatibility c4 identity retained an old runtime lane"
        )
    result.update(
        {
            "compatibility_policy_source_path": str(
            ...
            "c4_supervisor_file_sha256": _SELF_LOAD_SHA256,
...
def _verify_policy_compatibility_receipt(
    ...
    if runtime_phase not in _RUNTIME_PHASES:
        raise PermissionError("compatibility c4 runtime phase is invalid")
...
def _phase_guarded_inherited_callable(
    ...
    def guarded(*args, **kwargs):
        ...
        if selected not in _RUNTIME_PHASES - {"preactivation"}:
            raise PermissionError(
                "compatibility c4 runtime phase is invalid"
            )
    ...
    guarded.__name__ = getattr(function, "__name__", "guarded_c4_phase")
    guarded.__doc__ = getattr(function, "__doc__", None)
    return guarded
...
def _install_inherited_runtime_guards() -> None:
    ...
    must nevertheless cross the same c4 source-generation guard as this
...
# ensures calls reached through the frozen supervisor also cross the c4 guard.
