#!/usr/bin/env python3
"""Validate or execute the corrected v20 BFA-CMIF/PMOPE bounded-400 run.

``--validate-create-only`` verifies generated and static bindings without
claiming output, loading cached ``D_R`` tensors, constructing an optimizer,
or running the real-``D_R`` gate.  ``--run-once`` consumes the only fixed
seed-42, 10-by-40 BFA-CMIF attempt.  The command exposes no seed, budget,
output, retry, or resume option and never reads ``D_V`` or ``D_T``.
"""
...
from cure_lite.experiment.coverage_state_bfa_bounded_runner import (
...
INVALID_R1_RUN_ID = "cure_lite_bfa_cmif_v20_pmope_bounded_400_r1"
INVALID_R1_COMPLETE_FINGERPRINT = (
...
RUN_ID = "cure_lite_bfa_cmif_v20_pmope_bounded_400_r2"
OUTPUT_REPO_PATH = f"runs/irstd1k_stage_a_seed42/{RUN_ID}"
OUTPUT_PATH = _ROOT / OUTPUT_REPO_PATH
RUN_SCHEMA = "cure-lite-bfa-cmif-v20-pmope-bounded-400-run-v1"
VALIDATION_SCHEMA = (
    "cure-lite-bfa-cmif-v20-pmope-bounded-400-"
    "create-only-validation-v1"
)
ATTEMPT_SCHEMA = (
    "cure-lite-bfa-cmif-v20-pmope-bounded-400-attempt-v1"
)
FAILURE_SCHEMA = (
    "cure-lite-bfa-cmif-v20-pmope-bounded-400-failure-v1"
)
CHECKPOINT_SCHEMA = (
    "cure-lite-bfa-cmif-v20-pmope-bounded-400-checkpoint-v1"
)
DECISION_SCHEMA = (
    "cure-lite-bfa-cmif-v20-pmope-bounded-400-decision-v1"
)
...
def _verify_frozen_sources() -> dict[str, Path]:
    """Reuse the exact hash-bound v18 real-``D_R`` source contract."""

    return _pmope_cli._verify_frozen_sources()


def _implementation_binding() -> tuple[tuple[str, str], ...]:
    """Bind the BFA closure, this entrypoint, and execution wrappers."""
    ...
    for relative in extras:
        ...
        if (
            ...
            raise RuntimeError(
                f"BFA bounded implementation path changed: {relative}"
            )
...
def _static_config_payload(
    *,
    ...
    """Construct the immutable singleton seed-42 BFA configuration."""

    if (
        len(dataset_free_receipt_fingerprint) != 64
        or len(sealed_v18_receipt_fingerprint) != 64
    ):
        raise ValueError("BFA prerequisite fingerprint changed")
    ...
    if (
        ...
        raise RuntimeError("frozen BFA-CMIF parameter count changed")
    frozen_hashes = {
    ...
    return {
        ...
        "protocol_correction": {
            ...
            "predecessor_run_id": INVALID_R1_RUN_ID,
            "predecessor_complete_fingerprint": (
                INVALID_R1_COMPLETE_FINGERPRINT
            ),
            "predecessor_terminal_decision": (
                "BFA_CMIF_V20_DR_GATE_FAIL"
            ),
            "predecessor_scientific_interpretation": (
                "invalid_gate_annotation_not_method_result"
            ),
            "incorrect_r1_role_annotation": (
                "component_null_endpoint_targets_counted_as_background"
            ),
            "corrected_role_annotation": (
                "component_null_background_excludes_endpoint_target"
            ),
            "model_equation_changed": False,
            "objective_changed": False,
            "data_population_changed": False,
            "seed_changed": False,
        ...
        "bounded_decision": {
            "status": "not_run_in_static_config",
            "policy": (
                "predeclared-v20-structural-advancement-inequalities"
            ),
            "threshold_search_performed": False,
        },
        "implementation": {
...
def _device_memory_preflight(
    ...
    """Verify the sealed v18 cache/optimizer memory coordinates."""

    if type(model_config) is not CoverageStateBinaryFlipAntisymmetricConfig:
        raise TypeError("BFA memory preflight requires exact config class")
    if FROZEN_DEVICE != "cuda:0":
        raise RuntimeError("BFA bounded execution requires cuda:0")
    if (
        ...
        raise RuntimeError("BFA static memory model contract changed")
    ...
    if not result["all_pass"]:
        raise RuntimeError("BFA device memory preflight did not pass")
    return result
...
def _write_checkpoint_new(
    ...
    *,
    ...
    """Write and roundtrip the sole tensor-only BFA checkpoint."""

    if type(model) is not CURELiteBinaryFlipAntisymmetricLevelSet:
        raise TypeError("BFA checkpoint requires the exact model class")
    if objective != "pmope_joint" or objective_policy != CSLF_PMOPE_POLICY:
        raise ValueError("checkpoint must be the singleton BFA candidate")
    state = {
    ...
    if set(loaded) != set(state) or any(
        ...
        raise RuntimeError("BFA checkpoint roundtrip changed")
    result = _fingerprinted(
        {
            ...
            "model_config": {
                ...
                "interaction_policy": (
                    model.config.interaction_policy
                ),
...
def validate_create_only() -> dict[str, object]:
    ...
    dataset_free = run_coverage_state_bfa_dataset_free_gate()
    if not dataset_free.all_pass:
        raise RuntimeError("BFA dataset-free gate did not pass")
    sealed_v18 = verify_repository_coverage_state_uscope_sealed_v18(
        _ROOT
    )
    ...
    return _fingerprinted(
        {
            ...
            "protocol_correction_id": PROTOCOL_CORRECTION_ID,
            "predecessor_run_id": INVALID_R1_RUN_ID,
            "predecessor_is_method_result": False,
...
def _prepare_bfa_authorization(
    preflight: object,
    dataset_free: object,
    dr_gate: object,
    *,
    ...
    return prepare_coverage_state_bfa_bounded_run_authorization(
        preflight,
        dataset_free,
        dr_gate,
...
def _expected_bfa_config(
    ...
    return expected_coverage_state_bfa_config(preflight)


def _run_bfa_bounded(
    ...
    return run_coverage_state_bfa_cmif_pmope_bounded_400(
        authorization,
        model_config,
        device=FROZEN_DEVICE,
    )
    ...
    return _fingerprinted(
        {
            ...
            "bfa_zero_level_decision": decision_payload,
            "bfa_zero_level_decision_fingerprint": (
...
def _decision_payload(
    ...
    *,
    ...
    if (
        ...
        raise ValueError("BFA decision requires one candidate checkpoint")
    ...
    if (
        ...
        raise ValueError("BFA result fingerprint is invalid")
    return _fingerprinted(
        {
            ...
            "historical_result_is_candidate_gate": False,
            "next_action": (
                "freeze_bfa_v20_result_and_design_formal800_protocol"
                if passed
                else "freeze_bfa_v20_negative_result_and_review_structure"
            ),
...
def run_once() -> dict[str, object]:
    """Consume the sole fixed seed-42 BFA-CMIF bounded attempt."""
    ...
    dataset_free = run_coverage_state_bfa_dataset_free_gate()
    if not dataset_free.all_pass:
        raise PermissionError("BFA dataset-free gate did not pass")
    sealed_v18 = verify_repository_coverage_state_uscope_sealed_v18(
        _ROOT
    )
    if not _sealed_v18_is_read_only(sealed_v18.canonical_payload()):
        raise PermissionError(
            "sealed v18 negative result is not read-only"
        )
    source_paths = _verify_frozen_sources()
    implementation = _implementation_binding()
    ...
    try:
        ...
        dr_gate = run_coverage_state_bfa_dr_gate(
        ...
        if not dr_gate_passed:
            decision = _fingerprinted(
                {
                    "schema_version": DECISION_SCHEMA,
                    "status": "BFA_CMIF_V20_DR_GATE_FAIL",
                    ...
                    "performance_claim_supported": False,
                    "next_action": (
                        "freeze_bfa_D_R_gate_negative_result_and_stop_"
                        "before_bounded_training"
                    ),
                    "resume_allowed": False,
                    "automatic_retry_allowed": False,
                    "D_V_accessed": False,
            ...
            if (
                ...
                raise RuntimeError(
                    "BFA D_R gate-stop artifact population is incomplete"
                )
            complete = _fingerprinted(
                {
                    ...
                    "sealed_v18_receipt_fingerprint": (
                        sealed_receipt["receipt_fingerprint"]
                    ),
                    "D_R_gate_receipt_fingerprint": (
                        dr_gate_receipt["receipt_fingerprint"]
                    ),
        ...
        authorization = _prepare_bfa_authorization(
            preflight,
            dataset_free,
            dr_gate,
            sealed_v18_receipt=sealed_v18,
        )
        model_config = _expected_bfa_config(preflight)
        if (
            type(model_config)
            is not CoverageStateBinaryFlipAntisymmetricConfig
            or model_config.feature_channels
            ...
            raise RuntimeError("real D_R BFA model config changed")
        ...
        if not authorization.training_authorized:
            raise PermissionError("BFA authorization did not pass")
        ...
        result = _run_bfa_bounded(authorization, model_config)
        if (
            ...
            raise RuntimeError("BFA run returned a non-singleton result")
        ...
        training_result_fingerprint = stable_fingerprint(
            training_payload
        )
        training_receipt = _fingerprinted(
            {
                ...
                "candidate_objective": "pmope_joint",
                "historical_result_retrained": False,
                "all_models_exact_bfa_class": True,
        ...
        if _implementation_binding() != implementation:
            raise RuntimeError(
                "BFA implementation changed during execution"
            )

        artifacts = _v15b_cli._artifact_hashes(OUTPUT_PATH)
        if len(artifacts) != FROZEN_ARTIFACT_FILE_COUNT:
            raise RuntimeError(
                "BFA terminal artifact population is incomplete"
            )
        complete = _fingerprinted(
            {
                "schema_version": RUN_SCHEMA,
