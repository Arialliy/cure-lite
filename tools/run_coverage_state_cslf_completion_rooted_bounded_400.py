#!/usr/bin/env python3
"""Execute one real-D_R completion-rooted CSLF bounded-400 run.
...
RUN_ID = "cure_lite_cslf_v15a_completion_rooted_bounded_400_r1"
...
RUN_SCHEMA = "cure-lite-cslf-v15a-completion-rooted-bounded-400-run-v1"
ATTEMPT_SCHEMA = (
    "cure-lite-cslf-v15a-completion-rooted-bounded-400-attempt-v1"
)
FAILURE_SCHEMA = (
    "cure-lite-cslf-v15a-completion-rooted-bounded-400-failure-v1"
)
DECISION_SCHEMA = (
    "cure-lite-cslf-v15a-completion-rooted-bounded-400-decision-v1"
)
VALIDATION_SCHEMA = (
    "cure-lite-cslf-v15a-completion-rooted-bounded-400-"
...
CHECKPOINT_SCHEMA = (
    "cure-lite-cslf-v15a-completion-rooted-bounded-400-checkpoint-v1"
)

PARENT_V15_RUN_REPO_PATH = (
    "runs/irstd1k_stage_a_seed42/"
    "cure_lite_cslf_v15_bounded_400_r1"
)
PARENT_V15_COMPLETE_SHA256 = (
    "bf443854ec3137fd1507efb03cd7fda9dc2dfebef2ba22cb47c1818f20759437"
)
PARENT_V15_COMPLETE_FINGERPRINT = (
    "faaa2395623f5edfa0e56ab849d20305b73df1e7b3446b22b834279a2637d14b"
)
PARENT_V15_SOURCE_ARCHIVE_REPO_PATH = (
    "artifacts/source_closures/"
    "cure_lite_cslf_v15_bounded_400_faaa2395623f.tar"
)
PARENT_V15_SOURCE_ARCHIVE_SHA256 = (
    "5904c04a8ed2b4af28e46d159df25cf26bce956c4b80c27a2c488ad8a31491c7"
)
PARENT_V15_SOURCE_MANIFEST_REPO_PATH = (
    "artifacts/source_closures/"
    "cure_lite_cslf_v15_bounded_400_faaa2395623f.json"
)
PARENT_V15_SOURCE_MANIFEST_SHA256 = (
    "fe1667e159cfc9ee0b0eb0a9f941e118a83725386215257211608c6146baaa3c"
)
...
def _verify_parent_v15_closure() -> dict[str, object]:
    """Verify the immutable negative result and its exact source archive."""

    parent_dir = (_ROOT / PARENT_V15_RUN_REPO_PATH).resolve(strict=True)
    expected_dir = Path(
        os.path.abspath(_ROOT / PARENT_V15_RUN_REPO_PATH)
    )
    if (
        ...
        raise RuntimeError("parent v15 result directory changed")
    complete_path = _canonical_repo_file(
        f"{PARENT_V15_RUN_REPO_PATH}/COMPLETE.json",
        name="parent v15 COMPLETE",
    )
    if file_sha256(complete_path) != PARENT_V15_COMPLETE_SHA256:
        raise RuntimeError("parent v15 COMPLETE changed")
    complete = json.loads(complete_path.read_text(encoding="utf-8"))
    if (
        complete.get("complete_fingerprint")
        != PARENT_V15_COMPLETE_FINGERPRINT
        or complete.get("status") != "complete"
        or complete.get("decision") != "BOUNDED_CSLF_GATE_FAIL"
        or complete.get("bounded_gate_passed") is not False
    ):
        raise RuntimeError("parent v15 decision binding changed")
    artifact_files = complete.get("artifact_files")
    if not isinstance(artifact_files, dict) or not artifact_files:
        raise RuntimeError("parent v15 artifact map is missing")
    ...
    for relative, expected_sha256 in artifact_files.items():
        if (
            ...
            raise RuntimeError("parent v15 artifact map is invalid")
        ...
        if (
            ...
            raise RuntimeError(
                f"parent v15 artifact changed: {relative}"
            )
    ...
    if actual_artifacts != expected_artifacts:
        raise RuntimeError("parent v15 artifact population changed")
    if (
        (parent_dir / ".incomplete").exists()
        or (parent_dir / "FAILURE.json").exists()
    ):
        raise RuntimeError("parent v15 completion state changed")
    source_archive = _canonical_repo_file(
        PARENT_V15_SOURCE_ARCHIVE_REPO_PATH,
        name="parent v15 source archive",
    )
    source_manifest = _canonical_repo_file(
        PARENT_V15_SOURCE_MANIFEST_REPO_PATH,
        name="parent v15 source manifest",
    )
    if (
        ...
        raise RuntimeError("parent v15 source closure changed")
    return {
        "run_repo_path": PARENT_V15_RUN_REPO_PATH,
        "complete_file_sha256": PARENT_V15_COMPLETE_SHA256,
        "complete_fingerprint": PARENT_V15_COMPLETE_FINGERPRINT,
        "artifact_file_count": len(artifact_files),
        "source_archive_repo_path": (
            PARENT_V15_SOURCE_ARCHIVE_REPO_PATH
        ),
        "source_archive_sha256": PARENT_V15_SOURCE_ARCHIVE_SHA256,
        "source_manifest_repo_path": (
            PARENT_V15_SOURCE_MANIFEST_REPO_PATH
        ),
        "source_manifest_sha256": PARENT_V15_SOURCE_MANIFEST_SHA256,
    }
...
def _static_config_payload(
    *,
    ...
    parent_v15 = _verify_parent_v15_closure()
    return {
        ...
        "parent_v15_negative_result": parent_v15,
        "evidence_scope": {
            "D_V_accessed": False,
            "D_T_accessed": False,
    ...
    return _fingerprinted(
        {
            ...
            "status": (
                "BOUNDED_COMPLETION_ROOTED_CSLF_GATE_PASS"
                if passed
                else "BOUNDED_COMPLETION_ROOTED_CSLF_GATE_FAIL"
            ),
            ...
            "next_action": (
                ...
                if passed
                    ...
                    "freeze_completion_rooted_result_and_reassess_"
            ...
            "parent_v15_complete_fingerprint": (
                result.authorization.parent_v15_complete_fingerprint
            ),
            "formal_800_authorized": False,
...
def run_once() -> dict[str, object]:
    ...
    try:
        ...
        input_receipt = _fingerprinted(
            {
                "schema_version": (
                    "cure-lite-cslf-v15a-completion-rooted-"
        ...
        result = run_coverage_state_completion_rooted_bounded_400(
            authorization,
            model_config,
            device=FROZEN_DEVICE,
        )
        ...
        zero_receipt = _fingerprinted(
            {
                ...
                "candidate_bounded_gate_passed": dict(
                    result.checks
                )["candidate_zero_level_gates"],
        ...
        if _verify_parent_v15_closure() != parent_v15:
            raise RuntimeError("parent v15 closure changed")
