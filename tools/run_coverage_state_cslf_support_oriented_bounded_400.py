#!/usr/bin/env python3
"""Execute one real-D_R support-oriented CSLF bounded-400 run.

The bounded entry point has no configurable data path, output path, device,
...
RUN_ID = "cure_lite_cslf_v15b_support_oriented_bounded_400_r1"
...
RUN_SCHEMA = "cure-lite-cslf-v15b-support-oriented-bounded-400-run-v1"
ATTEMPT_SCHEMA = (
    "cure-lite-cslf-v15b-support-oriented-bounded-400-attempt-v1"
)
FAILURE_SCHEMA = (
    "cure-lite-cslf-v15b-support-oriented-bounded-400-failure-v1"
)
DECISION_SCHEMA = (
    "cure-lite-cslf-v15b-support-oriented-bounded-400-decision-v1"
)
VALIDATION_SCHEMA = (
    "cure-lite-cslf-v15b-support-oriented-bounded-400-"
...
CHECKPOINT_SCHEMA = (
    "cure-lite-cslf-v15b-support-oriented-bounded-400-checkpoint-v1"
)

PARENT_V15A_RUN_REPO_PATH = (
    "runs/irstd1k_stage_a_seed42/"
    "cure_lite_cslf_v15a_completion_rooted_bounded_400_r1"
)
PARENT_V15A_COMPLETE_SHA256 = (
...
_PACKAGE_IMPLEMENTATION_PATHS = tuple(
    sorted(
...
def _verify_parent_v15a_closure() -> dict[str, object]:
    """Verify the immutable negative result and its exact source archive."""

    parent_dir = (_ROOT / PARENT_V15A_RUN_REPO_PATH).resolve(strict=True)
    expected_dir = Path(
        os.path.abspath(_ROOT / PARENT_V15A_RUN_REPO_PATH)
    )
    if (
        parent_dir != expected_dir
        or not parent_dir.is_dir()
        or parent_dir.is_symlink()
    ):
        raise RuntimeError("parent v15A result directory changed")
    complete_path = _canonical_repo_file(
        f"{PARENT_V15A_RUN_REPO_PATH}/COMPLETE.json",
        name="parent v15A COMPLETE",
    )
    if file_sha256(complete_path) != PARENT_V15A_COMPLETE_SHA256:
        raise RuntimeError("parent v15A COMPLETE changed")
    complete = json.loads(complete_path.read_text(encoding="utf-8"))
    if (
        complete.get("complete_fingerprint")
        != PARENT_V15A_COMPLETE_FINGERPRINT
        or complete.get("status") != "complete"
        or complete.get("decision") != "BOUNDED_COMPLETION_ROOTED_CSLF_GATE_FAIL"
        or complete.get("bounded_gate_passed") is not False
    ):
        raise RuntimeError("parent v15A decision binding changed")
    artifact_files = complete.get("artifact_files")
    if (
        not isinstance(artifact_files, dict)
        or len(artifact_files) != PARENT_V15A_ARTIFACT_FILE_COUNT
    ):
        raise RuntimeError("parent v15A artifact map is missing")
    expected_artifacts = set()
    for relative, expected_sha256 in artifact_files.items():
        if (
            not isinstance(relative, str)
            or not isinstance(expected_sha256, str)
            or len(expected_sha256) != 64
        ):
            raise RuntimeError("parent v15A artifact map is invalid")
        ...
        if (
            ...
            raise RuntimeError(
                f"parent v15A artifact changed: {relative}"
            )
    ...
    if actual_artifacts != expected_artifacts:
        raise RuntimeError("parent v15A artifact population changed")
    if (
        (parent_dir / ".incomplete").exists()
        or (parent_dir / "FAILURE.json").exists()
    ):
        raise RuntimeError("parent v15A completion state changed")
    source_archive = _canonical_repo_file(
        PARENT_V15A_SOURCE_ARCHIVE_REPO_PATH,
        name="parent v15A source archive",
    )
    source_manifest = _canonical_repo_file(
        PARENT_V15A_SOURCE_MANIFEST_REPO_PATH,
        name="parent v15A source manifest",
    )
    if (
        file_sha256(source_archive)
        != PARENT_V15A_SOURCE_ARCHIVE_SHA256
        or file_sha256(source_manifest)
        != PARENT_V15A_SOURCE_MANIFEST_SHA256
    ):
        raise RuntimeError("parent v15A source closure changed")
    return {
        "run_repo_path": PARENT_V15A_RUN_REPO_PATH,
        "complete_file_sha256": PARENT_V15A_COMPLETE_SHA256,
        "complete_fingerprint": PARENT_V15A_COMPLETE_FINGERPRINT,
        "artifact_file_count": len(artifact_files),
        "source_archive_repo_path": (
            PARENT_V15A_SOURCE_ARCHIVE_REPO_PATH
        ),
        "source_archive_sha256": PARENT_V15A_SOURCE_ARCHIVE_SHA256,
        "source_manifest_repo_path": (
            PARENT_V15A_SOURCE_MANIFEST_REPO_PATH
        ),
        "source_manifest_sha256": PARENT_V15A_SOURCE_MANIFEST_SHA256,
    }
...
def _static_config_payload(
    *,
    ...
    parent_v15a = _verify_parent_v15a_closure()
    return {
        ...
        "execution": {
            ...
            "checkpoint_serialization": _checkpoint_backend(),
            "temperature_wrapper_repo_path": (
                TEMPERATURE_WRAPPER_REPO_PATH
            ),
            "temperature_wrapper_file_sha256": (
        ...
        "parent_v15a_negative_result": parent_v15a,
        "dataset_free_gate": {
...
def _validate_wrapper_command(tokens: Sequence[str]) -> None:
    ...
    if candidates != [wrapper]:
        raise RuntimeError("bounded run must be launched by the bound wrapper")
...
def _verify_runtime_contract() -> dict[str, object]:
    if os.environ.get("CUDA_VISIBLE_DEVICES") != FROZEN_VISIBLE_GPU:
        raise RuntimeError("bounded run fixes CUDA_VISIBLE_DEVICES=0")
    if (
        os.environ.get("CUBLAS_WORKSPACE_CONFIG")
        != FROZEN_CUBLAS_WORKSPACE_CONFIG
    ):
        raise RuntimeError(
            "bounded run fixes CUBLAS_WORKSPACE_CONFIG=:4096:8"
        )
    _validate_wrapper_command(_read_parent_command())
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        raise RuntimeError(
            "bounded run requires exactly one visible CUDA device"
        )
    if torch.cuda.current_device() != 0:
        raise RuntimeError("bounded run requires visible cuda:0")
...
def _checkpoint_backend() -> str:
    try:
        ...
        raise RuntimeError(
            "bounded run requires frozen safetensors checkpoint "
            "serialization"
        ) from error
    return FROZEN_CHECKPOINT_SERIALIZATION
...
def _write_checkpoint_new(
    ...
    *,
    ...
    if backend != FROZEN_CHECKPOINT_SERIALIZATION:
        raise RuntimeError("checkpoint serialization contract changed")
    ...
    return _fingerprinted(
        {
            ...
            "status": (
                "BOUNDED_SUPPORT_ORIENTED_CSLF_GATE_PASS"
                if passed
                else "BOUNDED_SUPPORT_ORIENTED_CSLF_GATE_FAIL"
            ),
            ...
            "next_action": (
                ...
                if passed
                    ...
                    "freeze_support_oriented_result_and_reassess_"
                    "absolute_zero_level_mechanism"
                )
            ),
            "candidate_objective": (
            ...
            "parent_v15a_complete_fingerprint": (
                result.authorization.parent_v15a_complete_fingerprint
            ),
            "formal_800_authorized": False,
            "full_CURE_authorized": False,
            "cross_backbone_authorized": False,
...
def _zero_level_payload(
    ...
    """Build the terminal zero-level receipt from the frozen result map."""
    ...
    return _fingerprinted(
        {
            "schema_version": (
                "cure-lite-cslf-v15b-support-oriented-"
...
def _complete_receipt_fingerprints(
    *,
    ...
    """Return the complete receipt graph for the terminal marker."""
...
def run_once() -> dict[str, object]:
    ...
    try:
        ...
        dataset_free = run_coverage_state_support_oriented_dataset_free_gate()
        if (
            dataset_free.receipt_fingerprint
            != FROZEN_SUPPORT_ORIENTED_DATASET_FREE_FINGERPRINT
        ):
            raise RuntimeError(
                "frozen support-oriented dataset-free receipt changed"
            )
        ...
        if _verify_parent_v15a_closure() != parent_v15a:
            raise RuntimeError("parent v15A closure changed")
